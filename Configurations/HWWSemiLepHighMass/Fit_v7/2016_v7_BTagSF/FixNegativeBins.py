...
signameFormatTAP = "histo_(top|qqWWqq|WW|ggH_hww|qqH_hww|ZH_hww|WH_hww|ggWW|Wjets|DY|Vg|VgS_L|VgS_H|VZ|VVV|ggH_htt|qqH_htt|WH_htt|ZH_htt|VBF_V|WWewk)_TagAndProbe_corr(Up|Down)"
...
signameFormatPSFSR = "histo_(top|qqWWqq|WW|ggH_hww|qqH_hww|ZH_hww|WH_hww|ggWW|Wjets|DY|Vg|VgS_L|VgS_H|VZ|VVV|ggH_htt|qqH_htt|WH_htt|ZH_htt)_PS_FSR(Up|Down)"
signameFormatpdfqqbar_sig = "histo_QQHSBI_([0-9]+)_RelW([0-9]+)_pdf_qqbar(Up|Down)"
signameFormatKFact = "histo_Wjets_Wjets_QCD_NLO_sf_stat_2016(Up|Down)"
...
signameFormatFake_el_recoil = "histo_FAKE_el_(barrel|endcap)_(pos|neg)_CMS_fake_e_(recoil|stat)_2016(Up|Down)"
signameFormatFake_mu_recoil = "histo_FAKE_mu_(barrel|endcap)_(pos|neg)_CMS_fake_m_(recoil|stat)_2016(Up|Down)"
...
#newfile2 = ROOT.TFile(rootfile.replace(".root", "_rel10new.root"), 'recreate')

for categories in newfile.GetListOfKeys(): # Cut directory
  if (categories.GetName() == 'histo_top'): 
    continue
  else: 
    ...
    for variables in category.GetListOfKeys(): # Variable directory
      ...
      #      histo.SetNameTitle(newname, newname)
          ...
          if not pattern == None:
            newname = "histo_Wjets_Wjets_QCD_NLO_sf_stat_"+topol+"_2016"+pattern.group(1)
            histo.SetNameTitle(newname, newname)
          ...
          if not pattern == None:
            newname = "histo_FAKE_el_"+pattern.group(1)+"_"+pattern.group(2)+"_CMS_fake_e_"+pattern.group(1)+"_"+pattern.group(2)+"_"+pattern.group(3)+"_2016"+pattern.group(4)
            histo.SetNameTitle(newname, newname)
          pattern = re.match(signameFormatFake_mu_recoil, histoname)
          if not pattern == None:
            newname = "histo_FAKE_mu_"+pattern.group(1)+"_"+pattern.group(2)+"_CMS_fake_m_"+pattern.group(1)+"_"+pattern.group(2)+"_"+pattern.group(3)+"_2016"+pattern.group(4)
            histo.SetNameTitle(newname, newname)
  ...
  #          histo.SetNameTitle(newname, newname)
          ...
          #if "data_obs" in histoname: histo.SetNameTitle(histoname.replace("data_obs", "DATA"), histoname.replace("data_obs", "DATA"))
