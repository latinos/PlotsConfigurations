...
##
###samples['top_semi_redo'] = {
###    'name': files,
###    'weight': mcCommonWeight,
###    'FilesPerJob': 1,
###    'EventsPerJob': 100000,
###}
###CombineBaseW(mcDirectory, samples, 'top_semi_redo',
###['TTWJetsToLNu_ext1', 'TTWJetsToLNu_ext2'])
###
###addSampleWeight(samples,'top','TTToSemiLeptonic','Top_pTrw')
###addSampleWeight(samples,'top_semi_redo','TTTo2L2Nu'       ,'Top_pTrw')
###addSampleWeight(samples,'top','TTWJetsToLNu_ext1'       ,'Top_pTrw')
###addSampleWeight(samples,'top_semi_redo','TTZjets'       ,'Top_pTrw')
##
...
#addSampleWeight(samples,'top','TTWJetsToLNu_ext1'       ,'Top_pTrw')
#addSampleWeight(samples,'top','TTWJetsToLNu_ext2'       ,'Top_pTrw')
...
samples['WWewk'] = {
    ...
    'weight': mcCommonWeightTagger + '*(Sum$(abs(GenPart_pdgId)==6 || GenPart_pdgId==25)==0)',
...
#
    ...
    files  += nanoGetSampleFiles(signalMCDirectory, 'GluGluHToWWToLNuQQ_M125') # will be weighted to S+I
#    files += nanoGetSampleFiles(signalMCDirectory, 'GluGluHToWWToLNuQQ_M500_copybkg') # just any mass, reweighted to HSM. But not the nominal 125 because that one has no off shell part
...
#    files += nanoGetSampleFiles(mcDirectory, 'WpToLNu_WmTo2J_QCD') # this is part of b with the same gen level selection as in qqWWqq
# 
#    samples['QQHSBI_te'+MX+model_name]  = {
#        'name': files,
#        'weight': mcCommonWeight,
#        'FilesPerJob': 10,
#    }
#    addSampleWeight(samples, 'QQHSBI_te'+MX+model_name, 'VBFHToWWToLNuQQ_M'+MX,
#                    '( (2*{0}+sqrt(2)*{1})*(abs({0}+{1})<50) )'.format(model, model_I))                
#    addSampleWeight(samples, 'QQHSBI_te'+MX+model_name, 'VBFHToWWToLNuQQ_M500_copybkg',
#                    '({0} )'.format(model_H))                
#    addSampleWeight(samples, 'QQHSBI_te'+MX+model_name, 'WpTo2J_WmToLNu_QCD',
#                    '(mjjGen_max>=150)*(GenLHE)')
#    addSampleWeight(samples, 'QQHSBI_te'+MX+model_name, 'WpToLNu_WmTo2J_QCD',
#                    '(mjjGen_max>=150)*(GenLHE)')
#
#    files  = nanoGetSampleFiles(signalMCDirectory, 'VBFHToWWToLNuQQ_M'+MX)
#    files += nanoGetSampleFiles(signalMCDirectory, 'VBFHToWWToLNuQQ_M500_copybkg') # just any mass, reweightestd to HSM. But not the nominal 125 because that one has no off shell part
#                                                                           # CAUTION when processing the 500 GeV signal.  We cannot have a sample with the same name weightestd in two different ways
#                                                                           # One solution is to symlink the 500 GeV signal with a different name in the directory (e.g. appending _copybgk to the name)                                                                               # and weight the copy as the background.
#    files += nanoGetSampleFiles(mcDirectory, 'WpTo2J_WmToLNu_QCD') # this is part of b with the same gen level selection as in qqWWqq
#    files += nanoGetSampleFiles(mcDirectory, 'WpToLNu_WmTo2J_QCD') # this is part of b with the same gen level selection as in qqWWqq
# 
...
#        'FilesPerJob': 10,
#    }
#    addSampleWeight(samples, 'QQHSBI_test'+MX+model_name, 'VBFHToWWToLNuQQ_M'+MX,
#                    '( (2*{0}+sqrt(2)*{1})*(abs(2*{0}+sqrt(2)*{1})<50) )'.format(model, model_I))                
#    addSampleWeight(samples, 'QQHSBI_test'+MX+model_name, 'VBFHToWWToLNuQQ_M500_copybkg',
#                    '({0} )'.format(model_H))                
...
#    files += nanoGetSampleFiles(signalMCDirectory, 'VBFHToWWToLNuQQ_M3000') # just any mass, reweightryd to HSM. But not the nominal 125 because that one has no off shell part
#                                                                           # CAUTION when processing the 500 GeV signal.  We cannot have a sample with the same name weightryd in two different ways
#                                                                           # One solution is to symlink the 500 GeV signal with a different name in the directory (e.g. appending _copybgk to the name)                                                                               # and weight the copy as the background.
    files += nanoGetSampleFiles(mcDirectory, 'WpTo2J_WmToLNu_QCD') # this is part of b with the same gen level selection as in qqWWqq
    files += nanoGetSampleFiles(mcDirectory, 'WpToLNu_WmTo2J_QCD') # this is part of b with the same gen level selection as in qqWWqq
    ...
    addSampleWeight(samples, 'QQHSBI_'+MX+model_name, 'VBFHToWWToLNuQQ_M'+MX,
                    '( (2*{0}*(abs({0})<50) + sqrt(2)*{1}*(abs({1})<50)) )'.format(model, model_I))                
#    addSampleWeight(samples, 'QQHSBI_try'+MX+model_name, 'VBFHToWWToLNuQQ_M3000',
#                    '({0} )'.format(model_H))                
    addSampleWeight(samples, 'QQHSBI_'+MX+model_name, 'WpTo2J_WmToLNu_QCD',
                    '(mjjGen_max>=150)*(GenLHE)')
    addSampleWeight(samples, 'QQHSBI_'+MX+model_name, 'WpToLNu_WmTo2J_QCD',
...
#    files =  nanoGetSampleFiles(signalMCDirectory, 'VBFHToWWToLNuQQ_M500_copybkg') # just any mass, reweightryd to HSM. But not the nominal 125 because that one has no off shell part
#                                                                           # CAUTION when processing the 500 GeV signal.  We cannot have a sample with the same name weightryd in two different ways
#                                                                           # One solution is to symlink the 500 GeV signal with a different name in the directory (e.g. appending _copybgk to the name)                                                                               # and weight the copy as the background.
#    files += nanoGetSampleFiles(mcDirectory, 'WpTo2J_WmToLNu_QCD') # this is part of b with the same gen level selection as in qqWWqq
#    files += nanoGetSampleFiles(mcDirectory, 'WpToLNu_WmTo2J_QCD') # this is part of b with the same gen level selection as in qqWWqq
 
#    samples['QQHSBI_see'+MX+model_name]  = {
#        'name': files,
#        'weight': mcCommonWeight,
#        'FilesPerJob': 10,
#    }
##    addSampleWeight(samples, 'QQHSBI_try'+MX+model_name, 'VBFHToWWToLNuQQ_M'+MX,
##                    '( ({0} + {1})*(abs({0})<50) )'.format(model, model_I))                
#    addSampleWeight(samples, 'QQHSBI_see'+MX+model_name, 'VBFHToWWToLNuQQ_M500_copybkg',
#                    '({0}*(abs({0}) < 50) )'.format(model_H))                
