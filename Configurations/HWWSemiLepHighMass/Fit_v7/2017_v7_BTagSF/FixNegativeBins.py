...
signameFormatKFact = "histo_Wjets_Wjets_QCD_NLO_sf_stat_2017(Up|Down)"
signameFormatQCDscale = "histo_Wjets_QCDscale_Wjets(Up|Down)"

signameFormatFake_el_recoil = "histo_FAKE_el_(barrel|endcap)_(pos|neg)_CMS_fake_e_(recoil|stat)_2017(Up|Down)"
signameFormatFake_mu_recoil = "histo_FAKE_mu_(barrel|endcap)_(pos|neg)_CMS_fake_m_(recoil|stat)_2017(Up|Down)"
try:
  rootfile = sys.argv[1]
except IndexError:
  ...
  for variables in category.GetListOfKeys(): # Variable directory
    ...
    for nom,var in sorted(histnames.iteritems()):
      ...
      for histoname in dothese:
        ...
        if not pattern == None:
          newname = "histo_Wjets_Wjets_QCD_NLO_sf_stat_"+topol+"_2017"+pattern.group(1)
          histo.SetNameTitle(newname, newname)
        ...
        if not pattern == None:
          newname = "histo_FAKE_el_"+pattern.group(1)+"_"+pattern.group(2)+"_CMS_fake_e_"+pattern.group(1)+"_"+pattern.group(2)+"_"+pattern.group(3)+"_2017"+pattern.group(4) 
          histo.SetNameTitle(newname, newname)
        pattern = re.match(signameFormatFake_mu_recoil, histoname)
        if not pattern == None:
          newname = "histo_FAKE_mu_"+pattern.group(1)+"_"+pattern.group(2)+"_CMS_fake_m_"+pattern.group(1)+"_"+pattern.group(2)+"_"+pattern.group(3)+"_2017"+pattern.group(4) 
          histo.SetNameTitle(newname, newname)
