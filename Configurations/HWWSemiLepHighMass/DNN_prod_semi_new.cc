...
//#ifndef DNNprodSemiNew_HH
//#define DNNprodSemiNew_HH

#include "NNEvaluation/DNNTensorflow/interface/DNNEvaluator.hh"
using namespace NNEvaluation;

class DNNprodSemiNew : public multidraw::TTreeFunction {
public:
  DNNprodSemiNew(unsigned int var);

  char const* getName() const override { return "DNNprodSemiNew"; }
  TTreeFunction* clone() const override { return new DNNprodSemiNew(_var); }
...
DNNprodSemiNew::DNNprodSemiNew(unsigned int var) :
  TTreeFunction()
{
  _var = var;
...
double
DNNprodSemiNew::evaluate(unsigned)
{
  ...
  if (_var == 1 ){ 
 	 if (ev % 2 == 0){
    		return dnn_tensorflow0->analyze(input)[1];
	  }else{
    		return dnn_tensorflow1->analyze(input)[1];
  	}
  }
  if (_var == 2 ){ 
 	 if (ev % 2 == 0){
    		return dnn_tensorflow0->analyze(input)[2];
	  }else{
    		return dnn_tensorflow1->analyze(input)[2];
  	}
  }
...
void
DNNprodSemiNew::bindTree_(multidraw::FunctionLibrary& _library)
{
