...
#        histograms_fr[fw].GetZaxis().SetRangeUser(0.0, 1.00)
#        canvas.SetLogy()
#        histograms_fr[fw].Draw('colz text')
...
for var in variables:
    ...
    for key in jet_et_dict:
        ...
        for idx,cut in enumerate(jet_et_dict[key]):
            ...
            histograms_fr[h_name].GetYaxis().SetRangeUser(0., 1.)
            ...
            if idx == 0: histograms_fr[h_name].Draw('text')
            else: histograms_fr[h_name].Draw('sametext')
        legend.Draw('same')
        canvas.Update()
        name = '_'.join(['plot', 'allJetEt', key, var])
        canvas.SaveAs(fr_dir+'/'+name+'bkg.png')
