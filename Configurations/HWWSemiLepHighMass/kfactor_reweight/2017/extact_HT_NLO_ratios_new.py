...
for year in run2:
    year_dict[year] = {}
    year_dict[year]['r_file'] = ROOT.TFile('Wjets_kfac_nano_root_Mar/plots_Wjets_kfac_'+year+'v7_nano_stud.root')
...
# Store
file_name = 'HT_to_NLO_QCD_k_factors_March.root'
r_file = ROOT.TFile.Open(file_name, 'RECREATE')
for year in run2:
    year_dict[year]['tg'] = ROOT.TH1D(year_dict[year]['ratio_s'])
    #year_dict[year]['tg_n'] = ROOT.TGraph(year_dict[year]['ratio_ns'])
    year_dict[year]['tg_up'] = ROOT.TH1D(year_dict[year]['ratio_s_up'])
    year_dict[year]['tg_do'] = ROOT.TH1D(year_dict[year]['ratio_s_do'])
   
    #year_dict[year]['tg'].SetDirectory(r_file)
    year_dict[year]['tg'].SetName('k_factor_2017_nom')#+year+'_nom')
    year_dict[year]['tg'].Write()
    #year_dict[year]['tg_n'].SetName('k_factor_'+year+'_jetsel')
    #year_dict[year]['tg_n'].Write()
    year_dict[year]['tg_up'].SetName('k_factor_2017_stat_up')#+year+'_stat_up')
    year_dict[year]['tg_up'].Write()
    year_dict[year]['tg_do'].SetName('k_factor_2017_down')#+year+'_stat_do')
