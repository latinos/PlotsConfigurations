...
if    'iihe' in SITE :
  ...
  treeBaseDir = '/eos/cms/store/caf/user/lenzip/'
...
directory = treeBaseDir+'Apr2017_summer16/lepSel__MCWeights__bSFLpTEffMulti__cleanTauMC__l2loose__hadd__l2tightOR__LepTrgFix__dorochester__formulasMC'+skim+'__doDNN/'

treeDNN_JH = '/eos/user/l/lusanche/Full2016/VBF_DNNvar/mjj700NodetajjDNN/'
directory2 = treeDNN_JH+'Apr2017_summer16/lepSel__MCWeights__bSFLpTEffMulti__cleanTauMC__l2loose__hadd__l2tightOR__LepTrgFix__dorochester__formulasMC'+skim+'__doDNN/' 
...
if useDYtt :
  ...
  if skim=='__wwSel':
    samples['DY']['name'] =   getSampleFiles(directory2,'DYJetsToTT_MuEle_M-50') \
                            + getSampleFiles(directory2,'DYJetsToTT_MuEle_M-50_ext1') \
                            + getSampleFiles(directory2,'DYJetsToLL_M-10to50')
  else:
    samples['DY']['name'] +=   getSampleFiles(directory2,'DYJetsToTT_MuEle_M-50') \
                             + getSampleFiles(directory2,'DYJetsToTT_MuEle_M-50_ext1')
...
samples['top'] = {   'name'     :   getSampleFiles(directory2,'TTTo2L2Nu') 
                                  + getSampleFiles(directory2,'ST_tW_antitop')
                                  + getSampleFiles(directory2,'ST_tW_top')  
                                  ...
                                  + getSampleFiles(directory2,'ST_t-channel_antitop')
                                  + getSampleFiles(directory2,'ST_t-channel_top')
                                  + getSampleFiles(directory2,'ST_s-channel')   
                             ,
                      'weight' : XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC ,  
                      'FilesPerJob' : 1 ,
                  }
...
samples['WW']  = {    'name'   : getSampleFiles(directory2,'WWTo2L2Nu') ,
                      'weight' : XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC + '*nllW' ,  
                 }

samples['WWewk'] = { 'name': getSampleFiles(directory2,'WpWmJJ_EWK_noTop'),
                     'weight' : XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC +'*(lhe_mWp>60)*(lhe_mWp<100)*(lhe_mWm>60)*(lhe_mWm<100)',
                   }

samples['ggWW']  = {  'name'   : getSampleFiles(directory2,'GluGluWWTo2L2Nu_MCFM'),      
                      'weight' : XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC ,  
                      'isData': ['0'],                            
                   }
...
samples['Vg']  =  {     'name'   :   getSampleFiles(directory2,'Wg_MADGRAPHMLM')
                                   + getSampleFiles(directory2,'Zg'),
...
samples['WZgS_L']  = {    'name': getSampleFiles(directory2,'WZTo3LNu_mllmin01_ext1') ,
                       'weight' : XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC + '* (Gen_ZGstar_mass >0 && Gen_ZGstar_mass < 4)*0.94' ,
                  }

samples['WZgS_H']  = {    'name': getSampleFiles(directory2,'WZTo3LNu_mllmin01_ext1') ,
                       'weight' : XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC + '* (Gen_ZGstar_mass <0 || Gen_ZGstar_mass > 4)*1.14' ,
                  } 

######### VZ #########

samples['VZ']  = {    'name':   getSampleFiles(directory2,'ZZTo2L2Nu')
                              + getSampleFiles(directory2,'WZTo2L2Q')
                              + getSampleFiles(directory2,'ZZTo2L2Q') , 
...
samples['VVV'] = {    'name':   getSampleFiles(directory2,'ZZZ')
                              + getSampleFiles(directory2,'WZZ')
                              + getSampleFiles(directory2,'WWZ')
                              + getSampleFiles(directory2,'WWW')
...
samples['ggH_hww']  = {  'name'  : getSampleFiles(directory2,'GluGluHToWWTo2L2NuPowheg_M125') ,  
...
samples['qqH_hww']  = {   'name' : getSampleFiles(directory2,'VBFHToWWTo2L2Nu_M125') ,
                         'weight': XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC ,  
                      }

### ZH ; H->WW

samples['ZH_hww']   = {  'name' :  getSampleFiles(directory2,'HZJ_HToWW_M125') ,
                         'weight': XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC ,  
                         'suppressNegativeNuisances' :['all'],
                      }

samples['ggZH_hww'] = {   'name' : getSampleFiles(directory2,'ggZH_HToWW_M125') ,
                          'weight': XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC ,  
                          'suppressNegativeNuisances' :['all'],
                      }
...
samples['WH_hww']   = {   'name' :   getSampleFiles(directory2,'HWminusJ_HToWW_M125')
                                   + getSampleFiles(directory2,'HWplusJ_HToWW_M125')
                                   , 
                         'weight': XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC ,  
                      }
...
samples['bbH_hww']  = {  'name' :   getSampleFiles(directory2,'bbHToWWTo2L2Nu_M125_yb2') 
                                  + getSampleFiles(directory2,'bbHToWWTo2L2Nu_M125_ybyt'),
                         'suppressNegativeNuisances' :['all'],
                         'weight': XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC ,
                      }
...
samples['ttH_hww']  = {  'name' :   getSampleFiles(directory2,'ttHToNonbb_M125'), 
                         'weight': XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC ,
                         'suppressNegativeNuisances' :['all'], 
                      }
...
if not splitHtt:
  ...
  samples['H_htt']    = {   'name' :   getSampleFiles(directory2,'GluGluHToTauTau_M125')
                                     + getSampleFiles(directory2,'VBFHToTauTau_M125')
                                     + getSampleFiles(directory2,'HZJ_HToTauTau_M125')
                                     + getSampleFiles(directory2,'HWplusJ_HToTauTau_M125')
                                     + getSampleFiles(directory2,'HWminusJ_HToTauTau_M125') ,  
                           'weight': XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC ,  
                           'suppressNegative' :['all'],
                           'suppressNegativeNuisances' :['all'],
                        }
  ...
  samples['ggH_htt']  = { 'name' :   getSampleFiles(directory2,'GluGluHToTauTau_M125') ,
                           'weight': XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC ,
                           'suppressNegative' :['all'],
                           'suppressNegativeNuisances' :['all'],
                        }

  samples['qqH_htt']  = { 'name' :   getSampleFiles(directory2,'VBFHToTauTau_M125') ,
                           'weight': XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC ,
                           'suppressNegative' :['all'],
                           'suppressNegativeNuisances' :['all'],
                        }
  ...
  samples['ZH_htt']  = { 'name' :   getSampleFiles(directory2,'HZJ_HToTauTau_M125') ,
                           'weight': XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC ,
                           'suppressNegative' :['all'],
                           'suppressNegativeNuisances' :['all'],
                        }

  samples['WH_htt']  = { 'name' :   getSampleFiles(directory2,'HWplusJ_HToTauTau_M125')
                                  + getSampleFiles(directory2,'HWminusJ_HToTauTau_M125'),
                           'weight': XSWeight+'*'+SFweight+'*'+GenLepMatch+'*'+METFilter_MC ,
                           'suppressNegative' :['all'],
                           'suppressNegativeNuisances' :['all'],
...
for Run in DataRun :
  #directory = treeBaseDir+'Apr2017_Run2016'+Run[0]+'_RemAOD/lepSel__EpTCorr__TrigMakerData__cleanTauData__l2loose__dorochester__multiFakeW__formulasFAKE__hadd'+skimFake+'__doDNN/'
  directory = treeDNN_JH+'Apr2017_Run2016'+Run[0]+'_RemAOD/lepSel__EpTCorr__TrigMakerData__cleanTauData__l2loose__dorochester__multiFakeW__formulasFAKE__hadd'+skimFake+'__doDNN/'
...
for Run in DataRun :
  #directory = treeBaseDir+'Apr2017_Run2016'+Run[0]+'_RemAOD/lepSel__EpTCorr__TrigMakerData__cleanTauData__l2loose__hadd__l2tightOR__dorochester__formulasDATA'+skim+'__doDNN/'
  directory = treeDNN_JH+'Apr2017_Run2016'+Run[0]+'_RemAOD/lepSel__EpTCorr__TrigMakerData__cleanTauData__l2loose__hadd__l2tightOR__dorochester__formulasDATA'+skim+'__doDNN/'
  for DataSet in DataSets :
    FileTarget = getSampleFiles(directory,DataSet+'_'+Run[1],True)
    for iFile in FileTarget:
