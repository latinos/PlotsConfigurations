...
def load_dataset_vbf ( max_entries = -1 ):
  _branches = [
    ...
    #"CleanJet_phi[0]",
    #"CleanJet_phi[1]",
    #"Alt$(CleanJet_pt[2]*(CleanJet_pt[2]>30), -1) - Alt$((CleanJet_pt[2]<=30), 0)",
    #"Alt$(CleanJet_eta[2]*(CleanJet_pt[2]>30), -1) - Alt$((CleanJet_pt[2]<=30), 0)",
    ...
    #"Alt$(mlj(CleanJet_pt[0], CleanJet_eta[0], CleanJet_phi[0], CleanJet_pt[2], CleanJet_eta[2], CleanJet_phi[2])*(CleanJet_pt[2]>30), -1) - Alt$((CleanJet_pt[2]<=30), 0)",
    #"Alt$(mlj(CleanJet_pt[1], CleanJet_eta[1], CleanJet_phi[1], CleanJet_pt[2], CleanJet_eta[2], CleanJet_phi[2])*(CleanJet_pt[2]>30), -1) - Alt$((CleanJet_pt[2]<=30), 0)",
    "log((abs(2*Lepton_eta[0]-CleanJet_eta[0]-CleanJet_eta[1])+abs(2*Lepton_eta[1]-CleanJet_eta[0]-CleanJet_eta[1]))/detajj)",
    #"log10(Alt$((Sum$(CleanJet_pt>30)>2)*abs(2*CleanJet_eta[2]-CleanJet_eta[1]-CleanJet_eta[0])/detajj+10*(Sum$(CleanJet_pt>30)<=2), 10))",
    "CleanJet_jetIdx[0]",
    "CleanJet_jetIdx[1]", 
    "Jet_qgl",  
    ]
  ...
  _dataset = root_numpy.tree2array (chain,
      branches = _branches,
      selection = 'mll>12 && PuppiMET_pt>20 && (mth>=60 && mth<125) && Lepton_pt[0]>25 && Lepton_pt[1]>13 && Alt$(Lepton_pt[2],0)<10 && ptll>30 && Sum$(CleanJet_pt>30)==2 && mjj>120 && (abs(CleanJet_eta[0])<4.7) && (abs(CleanJet_eta[1])<4.7) && Sum$(CleanJet_pt > 20. && abs(CleanJet_eta) < 2.5 && Jet_btagDeepB[CleanJet_jetIdx] > 0.2217) == 0 ',
      stop = max_entries
     )
...
def load_dataset_top ( max_entries = -1 ):
  ...
  chain = ROOT.TChain('Events')
  ''' 
  for i in range(0,22,1): #21 up to 73
  ...
  chain.Add(dir17+'nanoLatino_TTTo2L1Nu_PSWeights_CP5Down__part6.root')
...
def load_dataset_ww ( max_entries = -1 ):
  ...
  '''
  chain.Add(dir16+'nanoLatino_WpWmJJ_EWK_noTop__part0.root')
  '''
  chain.Add(dir17+'nanoLatino_WpWmJJ_EWK_noTop__part0.root')
  
  '''

  chain.Add(dir18+'nanoLatino_WpWmJJ_EWK_noTop__part0.root')
  chain.Add(dir18+'nanoLatino_WpWmJJ_EWK_noTop__part1.root')  
  ...
  _dataset = root_numpy.tree2array (chain, 
      selection = 'mll>12 && PuppiMET_pt>20 && (mth>=60 && mth<125) && Lepton_pt[0]>25 && Lepton_pt[1]>13 && Alt$(Lepton_pt[2],0)<10 && ptll>30 && Sum$(CleanJet_pt>30)==2 && mjj>120 && (abs(CleanJet_eta[0])<4.7) && (abs(CleanJet_eta[1])<4.7) && Sum$(CleanJet_pt > 20. && abs(CleanJet_eta) < 2.5 && Jet_btagDeepB[CleanJet_jetIdx] > 0.2217) == 0 && (Sum$(GenPart_pdgId==25)==0)',
      branches = _branches,
      stop = max_entries
     )
...
def load_dataset_ggh ( max_entries = -1 ):
  ...
  chain.Add(dir16+'nanoLatino_GluGluHToWWTo2L2NuPowheg_M125__part0.root')
  chain.Add(dir16+'nanoLatino_GluGluHToWWTo2L2NuPowheg_M125__part1.root')
  #chain.Add(dir16+'nanoLatino_GluGluHToWWTo2L2NuPowheg_M125__part2.root')
  #chain.Add(dir16+'nanoLatino_GluGluHToWWTo2L2NuPowheg_M125__part3.root')
  ...
  chain.Add(dir18+'nanoLatino_GluGluHToWWTo2L2NuPowheg_M125__part0.root')
  ...
  #for i in range(0,63,1): #up to 63
