...
def load_dataset_vbf ( max_entries = -1 ):
  ...
  _dataset = root_numpy.tree2array (chain,
      branches = _branches,
      selection = 'detajj>3.5 && mll>12 && PuppiMET_pt>20 && (mth>=60 && mth<125) && Lepton_pt[0]>25 && Lepton_pt[1]>10 && Alt$(Lepton_pt[2],0)<10 && ptll>30 && Sum$(CleanJet_pt>30)==2 && mjj>200 && (abs(CleanJet_eta[0])<4.7) && (abs(CleanJet_eta[1])<4.7) && Sum$(CleanJet_pt > 20. && abs(CleanJet_eta) < 2.5 && Jet_btagDeepB[CleanJet_jetIdx] > 0.2217) == 0',
      stop = max_entries
     )
