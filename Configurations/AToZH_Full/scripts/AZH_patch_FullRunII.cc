...
class AZH_patch_FullRunII : public multidraw::TTreeFunction {
    public:
	AZH_patch_FullRunII(std::string year, const char* variable_);

	char const* getName() const override { return "AZH_patch_FullRunII"; }
	TTreeFunction* clone() const override { return new AZH_patch_FullRunII(year, variable.c_str()); }
...
AZH_patch_FullRunII::AZH_patch_FullRunII(const std::string year_, const char* variable_) :  //constructor
    TTreeFunction(), year{year_}, variable{variable_} {
...
double
AZH_patch_FullRunII::evaluate(unsigned)
{
    int iL = getXLepton();
...
for (unsigned ij = 0; ij < nJetLoose; ij++){
	...
	nJet++;
	if ((std::abs(CleanJet_eta->At(ij)) < 2.5) && (Jet_btagDeepFlavB->At(CleanJet_jetIdx->At(ij)) > btagWP_map_[year])) nbJet++; }
}
...
int
AZH_patch_FullRunII::getXLepton(){
...
void
AZH_patch_FullRunII::bindTree_(multidraw::FunctionLibrary& _library)
{
