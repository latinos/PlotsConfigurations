...
void plotrvalues_withsyst() {
std::vector<float> r_valuesh1;
std::vector<float> r_valuesh2;
std::vector<float> r_valuesh3;
for (int i = 1; i < 1000; i++) {
     TFile* _fileh1 = new TFile(((std::string("condor_toys/multidimfitresult_1000_600_0signal0.5ttZ")+std::to_string(i)+std::string(".root")).c_str()));
     TFile* _fileh2 = new TFile(((std::string("condor_toys/multidimfitresult_1000_600_0signal1ttZ")+std::to_string(i)+std::string(".root")).c_str()));
     TFile* _fileh3 = new TFile(((std::string("condor_toys/multidimfitresult_1000_600_0signal1.5ttZ")+std::to_string(i)+std::string(".root")).c_str()));
    //_file0->cd();
...
TH1F* h1 = new TH1F("h1", "postfit signal strength", 20, -0.05, 0.05);
...
legend->Draw();
c1->Update();
c1->SaveAs("signalstrength_withsyst.png");
}
