...
class elliptical_bin_2016noHIPM : public multidraw::TTreeFunction {
public:
  elliptical_bin_2016noHIPM(const float mA_, const float mH_, const bool isonebjet_);
  char const* getName() const override { return "elliptical_bin_2016noHIPM"; }
  TTreeFunction* clone() const override { return new elliptical_bin_2016noHIPM(mA, mH, isonebjet); }
...
elliptical_bin_2016noHIPM::elliptical_bin_2016noHIPM(const float mA_, const float mH_, const bool isonebjet_) :
  TTreeFunction(), mA{mA_}, mH{mH_}, isonebjet{isonebjet_}
{
  ...
  std::string filename;
  if (isonebjet) filename = cmssw_base + "/src/PlotsConfigurations/Configurations/AToZH_Full/scripts/ellipse_onebjet_16noHIPM.txt"; //TEMP update with correct filename
  else           filename = cmssw_base + "/src/PlotsConfigurations/Configurations/AToZH_Full/scripts/ellipse_breq_16noHIPM.txt";
...
double elliptical_bin_2016noHIPM::evaluate(unsigned){
  double dM;
...
void elliptical_bin_2016noHIPM::bindTree_(multidraw::FunctionLibrary& _library)
{
