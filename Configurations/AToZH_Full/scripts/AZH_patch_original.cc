...
AZH_patch::evaluate(unsigned)
{
  ...
  for (unsigned ij = 0; ij < nJetLoose; ij++){ 
	  ...
	  TLorentzVector j; 
	  j.SetPtEtaPhiM(CleanJet_pt->At(ij), CleanJet_eta->At(ij), CleanJet_phi->At(ij), 0);
	  CleanJet_4vecId.push_back(j);
	  if (Jet_btagDeepB->At(CleanJet_jetIdx->At(ij)) > 0.4941) {
  ...
  //     std::cout<<"blahhhhhhh print"<< std::endl;  

      for (int ibJet1 = 0; ibJet1 != AZH_bJet_4vecId.size(); ibJet1++){
	    for (int ibJet2 = ibJet1 +1; ibJet2 != AZH_bJet_4vecId.size(); ibJet2++){
//		std::vector<TLorentzVector> WJets;
		for (unsigned ij = 0; ij < CleanJet_4vecId.size(); ij++){ 
		    if ((CleanJet_4vecId.at(ij) != AZH_bJet_4vecId.at(ibJet1)) && (CleanJet_4vecId.at(ij) != AZH_bJet_4vecId.at(ibJet2))) {
			WJets.push_back(CleanJet_4vecId.at(ij));
		    }
		}



		TLorentzVector bJetPair[2] = {AZH_bJet_4vecId.at(ibJet1), AZH_bJet_4vecId.at(ibJet2)};
		for(int i = 0; i < 2; i++){                   
		    for (int iWJet1 = 0; iWJet1 != WJets.size(); iWJet1++){
			for (int iWJet2 = iWJet1 + 1; iWJet2 != WJets.size(); iWJet2++){ 
			    TLorentzVector WJet1 = WJets.at(iWJet1);
			    TLorentzVector WJet2 = WJets.at(iWJet2);
			    TLorentzVector bJetHadronic = bJetPair[i];
			    TLorentzVector bJetLeptonic = bJetPair[1-i];
				...
				std::cout<< "WJet1" <<std::endl;  
				WJet1_best.Print();
				std::cout<<"WJet2"<<std::endl;
				WJet2_best.Print();
				std::cout<<"bJetHadronic"<< std::endl;
				bJetHadronic_best.Print();
				std::cout<<"bJetLeptonic"<< std::endl;
				bJetLeptonic_best.Print();
				std::cout<<"Neutrino"<< std::endl;
				AZH_Neutrino_best.Print();
				std::cout<<"ChiSquare"<< ChisqMin <<std::endl;
  ...
  else if (variable == "AZH_Whadronic") {
      if (nJet < 4 || nbJet < 2) return -9999.0;
      return (WJet1_best + WJet2_best).M();
   }

  else if (variable == "AZH_Wleptonic") {
      if (nJet < 4 || nbJet < 2) return -9999.0;
      return (XLepton + AZH_Neutrino_best).M();
   }

//  else if (variable == "Pt_notZl"){
//     return XLepton.Pt()
//  }
  ...
  else if (variable == "AZH_Zeta") {
       if (nJet < 4 || nbJet < 2) return -9999.0;
       return (Zeta);
  }

  else if (variable == "AZH_A") {
      if (nJet < 4 || nbJet < 2) return -9999.0;
      return A;
   }

  else if (variable == "AZH_Pznu1") {
      if (nJet < 4 || nbJet < 2) return -9999.0;
      return Pznu1;
   }

  else if (variable == "AZH_Pznu2") {
      if (nJet < 4 || nbJet < 2) return -9999.0;
      return Pznu2;
  }

  else if (variable == "AZH_Enu1") {
      if (nJet < 4 || nbJet < 2) return -9999.0;
      return Enu1;
   }

  else if (variable == "AZH_Enu2") {
      if (nJet < 4 || nbJet < 2) return -9999.0;
      return Enu2;
   }

  else if (variable == "AZH_nWJets") {
      return WJets.size();
   }

  else if (variable == "AZH_nbJets") {
      return AZH_bJet_4vecId.size();
   }

  else if (variable == "AZH_nbjet") {
      return nbJet;
   }
