...
with open("signals_test.txt", 'r') as f:
        ...
        for j in range(len(CleanJet_pt[i])):
            ...
            if (pt > 30 and abs(eta) < 2.5):
                ...
                if (jf == 0):
                    ...
                    #sf = 1.35875+(-0.000916722*pt)+(6.33425e-07*pt*pt)+(-2.07301/pt) # 2017
                elif ((jf == 4) | (jf ==5) ):
                    sf = 0.764066+(0.00814871*np.log(pt+19.0)*np.log(pt+18.0)*(3.0-(0.399988*np.log(pt+18.0)))) # 2018
                    #sf = 0.882658+(0.00425140*np.log(pt+19.0)*np.log(pt+18.0)*(3.0-(0.411282*np.log(pt+18.0))))  # 2017
...
####################### SR cuts ################################################################
    ...
    for i in range(len(Lepton_pt)):
        if (len(Lepton_pt[i]) < 3):
            ...
            WJet2_best = ROOT.TLorentzVector()
            bJetHadronic_best = ROOT.TLorentzVector()
...
#################################################################################################
    ...
    def _fit_ellipse(x, y, w, n_std):
       ...
       # print(pairs[b][0], pairs[b][1], mean_x, mean_y, width, height, angle, n_std)
        ...
        #print(lines)
        line.append(lines)
        tmparray = np.array([mean_x,mean_y,width,height,angle])
        plot_var.append(tmparray)
    ...
    tick_args = {"reset": True, "labelbottom": False, "labelleft": False}
    ax_histx.tick_params(**tick_args)
    ax_histy.tick_params(**tick_args)
    ax.tick_params(direction="in")

    ax.hexbin(ptz_cuts, dm_cuts, gridsize=35, cmap="Blues", vmin=0.1)
    ax.set_facecolor(plt.get_cmap("Blues")(0))
    for plotvar, color in zip(plot_var, ell_colors):
        ax.add_patch(mpatches.Ellipse(xy=(plotvar[0],plotvar[1]), width=plotvar[2], height=plotvar[3], angle=plotvar[4], ec=color, fc="None"))

    # 1D Plots
    binwidth = 25
    xymax = np.max(np.abs(ptz_cuts))
    lim = (int(xymax / binwidth) + 1) * binwidth

    bins = np.arange(-lim, lim + binwidth, binwidth)
    ax_histx.hist(ptz_cuts, bins=bins)
    ax_histy.hist(dm_cuts, bins=2 * bins, orientation="horizontal")
    ...
    ax.legend(
        [
            mpatches.Ellipse(xy=(plotvar[0],plotvar[1]), width=plotvar[2], height=plotvar[3], angle=plotvar[4], ec=color, fc="None")
            for plotvar, color in zip(plot_var, ell_colors)
        ],
        [rf"{n_std}$\sigma$" for n_std in binning_map["stddevs_ellipses"]],
        fontsize=18,
    )
    ax.tick_params(axis="both", which="major", labelsize=18)
    #plt.title("mA"+str(pairs[b][0])+"_mH"+str(pairs[b][1])+" before genAReq")
    fname_out = "/eos/user/m/mihawksw/azh/ellipses_2D_2018/mA"+str(pairs[b][0])+"_mH"+str(pairs[b][1])
    plt.savefig(fname_out + ".png")
    plt.savefig(fname_out + ".pdf")
    plt.close()

    
    #outfile = open('ellipse_breq_18.txt', 'a')
