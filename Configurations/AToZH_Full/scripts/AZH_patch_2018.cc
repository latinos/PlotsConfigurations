...
using std::cout;
using std::endl;
float bWP_2018 = 0.2783;
using PtEtaPhiMVector = ROOT::Math::PtEtaPhiMVector;
class AZH_patch_2018 : public multidraw::TTreeFunction {
    public:
	AZH_patch_2018(const char* variable_);

	char const* getName() const override { return "AZH_patch_2018"; }
	TTreeFunction* clone() const override { return new AZH_patch_2018(variable.c_str()); }
...
AZH_patch_2018::AZH_patch_2018(const char* variable_) :  //constructor
    TTreeFunction(),
    variable{variable_}
{}

double
AZH_patch_2018::evaluate(unsigned)
{
    int iL = getXLepton();
	...
	if (CleanJet_pt->At(z) > 30 && std::abs(CleanJet_eta->At(z)) < 4.7){ 
	    ...
	    if (Jet_btagDeepFlavB->At(CleanJet_jetIdx->At(z)) > bWP_2018) {
...
for (unsigned ij = 0; ij < nJetLoose; ij++){
	...
	nJet++;
	if (Jet_btagDeepFlavB->At(CleanJet_jetIdx->At(ij)) > bWP_2018) nbJet++; }
}
...
void
AZH_patch_2018::bindTree_(multidraw::FunctionLibrary& _library)
{
