...
class elliptical_bin : public multidraw::TTreeFunction {
public:
  elliptical_bin(const float mA_, const float mH_, const bool isonebjet_);
  char const* getName() const override { return "elliptical_bin"; }
  TTreeFunction* clone() const override { return new elliptical_bin(mA, mH, isonebjet); }
...
elliptical_bin::elliptical_bin(const float mA_, const float mH_, const bool isonebjet_) :
