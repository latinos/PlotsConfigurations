...
#path and prefix below must match those in condor_fits.sub and condor_resubmit_fits.sub
folder_path="jobs_fit"
file_prefix="fit"

error_file_numbers=()
while IFS= read -r -d '' file; do
    if (! grep -q "Expected [0-9]*\.[0-9]%: r <" "$file" || ! grep -q "Significance:" "$file"); then
...
sed -e "s/TMP/${joined_numbers}/g" "condor_resubmit_fits.sub" > "condor_resubmit_fits_tmp.sub"
condor_submit "condor_resubmit_fits_tmp.sub"
rm "condor_resubmit_fits_tmp.sub"
