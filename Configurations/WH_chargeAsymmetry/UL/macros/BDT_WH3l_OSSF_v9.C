...
class BDT_WH3l_OSSF_v9 : public multidraw::TTreeFunction {
public:
  BDT_WH3l_OSSF_v9(TString BDT_name, TString xml_file_name);

  char const* getName() const override { return "BDT_WH3l_OSSF_v9"; }
  TTreeFunction* clone() const override { return new BDT_WH3l_OSSF_v9(BDT_name_, xml_file_name_); }
...
protected:
  ...
  FloatValueReader* WH3l_ZVeto{};
  FloatArrayReader* WH3l_mtlmet{};
...
BDT_WH3l_OSSF_v9::BDT_WH3l_OSSF_v9(TString BDT_name, TString xml_file_name) : TTreeFunction() {
  ...
  // We also need to introduce an era variable to choose the input xml file

  // reader->BookMVA("BDTG4F07","/afs/cern.ch/user/n/ntrevisa/work/latinos/unblinding/CMSSW_10_6_4/src/PlotsConfigurations/Configurations/WH_chargeAsymmetry/UL/Full2018_v9/BDTconfig_WH3l/datasetOSSF/weights/TMVAClassification_BDTG4F07.weights.xml");
  reader->BookMVA(BDT_name, xml_file_name);
}

// Evaluate the BDT
double BDT_WH3l_OSSF_v9::evaluate(unsigned){
  ...
  WH3l_ZVeto_       = *WH3l_ZVeto->Get();
  WH3l_mtlmet_1_    = WH3l_mtlmet->At(1);
  WH3l_mtlmet_2_    = WH3l_mtlmet->At(2);
  ...
  // double classifier = reader->EvaluateMVA("BDTG4F07");
...
void BDT_WH3l_OSSF_v9::bindTree_(multidraw::FunctionLibrary& _library){
  ...
  _library.bindBranch(WH3l_ZVeto,      "WH3l_ZVeto");
  _library.bindBranch(WH3l_mtlmet,     "WH3l_mtlmet");
