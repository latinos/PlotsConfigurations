...
#parser.add_option('--signal_scaling',   dest='signal_scaling',   help='signal scaling: x1 or x10',             default="10")

(opt, args) = parser.parse_args()

print("Datacard name    = {}".format(opt.datacard_name))
print("Output name      = {}".format(opt.output_name))
print("Do sanity checks = {}".format(opt.sanity_check))
print("Freeze nuisances = {}".format(opt.freeze_nuisances))
#print("Signal scaling   = {}".format(opt.signal_scaling))
...
rA_scan_command = "combine \
                   ...
                   --algo grid \
                   -t -1 \
                   --setParameters r_A=0.224,r_S=1.3693,r_higgs=1 \
                   -d {0}.root \
                   -n _r_A_scan \
                   --points 20 \
...
if (opt.freeze_nuisances) != False:
    root_output_name = output_name.replace(".txt","_freeze_{}.root".format(opt.freeze_nuisances))
...
if sanity_check != False:
    ...
    if "ML" in sanity_check:
        ...
        if (opt.freeze_nuisances) != False:
            ML_output_name = output_name.replace(".txt","_ML_freeze_{}.root".format(opt.freeze_nuisances))
    ...
    if "rA_scan" in sanity_check:
        ...
        if (opt.freeze_nuisances) != False:
            rA_output_name = output_name.replace(".txt","_rA_freeze_{}.root".format(opt.freeze_nuisances))
