...
//   if (den != 0) hRatioDo -> SetBinError  (iBin+1, 1./den * hRatioDo->GetBinError(iBin+1));
 ...
 float rounded_max_ratio = int( 10. * (max_ratio + 0.1) ) / 10.;
 hReferenceRatio -> GetYaxis() -> SetRangeUser(1 - abs(rounded_max_ratio-1), rounded_max_ratio);
 cout << "Ratio: "<< rounded_max_ratio <<endl;
 hReferenceRatio->Draw();
 hRatioUp->Draw("same");
 hRatioDo->Draw("same");
