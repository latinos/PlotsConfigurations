...
for sample in mc :
    if sample in ["ggWW","VBS","VBS_dipoleRecoil","Wjets_boost","VV"] + wjets_res_bins + VBS_samples + VV_samples : continue
...
samples_PS = ['top','DY','VVV','Vg','VgS','VBF-V_dipole','ggWW'] + wjets_bins #VBS_dipoleRecoil'VV',
...
#     'type'  : 'shape',
#     'samples' :  { s: [' Alt$(LHEPdfWeight['+str(i)+'], 1.)' for i in range(0,103)] for s in mc if s not in ["VBS","VBS_dipoleRecoil", "VV", "top","Wjets_boost"]+wjets_res_bins + VBS_samples + VV_WV_samples},
#     'AsLnN':  '1'
# }
