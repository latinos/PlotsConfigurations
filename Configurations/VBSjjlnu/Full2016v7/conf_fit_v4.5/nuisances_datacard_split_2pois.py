...
mc =["DY", "top", "VV", "VVV", "Vg", "VgS", "VBS_dipoleRecoil", "VBF-V_dipole", "ggWW","Wjets_boost"] + wjets_res_bins + VBS_samples + VV_samples
...
#                         'AsLnN'      : '1',               
#         }
        #This is only for W+jets boost and it is done already in the entry with other mc
        # nuisances['fatjet' +js + "_" +wjbin]  = {
        #                 'name': 'CMS_fj_scale_'+js,
        #                 'kind': 'suffix',
        #                 'type': 'shape',
        #                 'mapUp': 'fatjet' + js+'up',
        ...
        #                 'cuts': phase_spaces_boost, #because we are vetoing fatjets anyway in resolved category 
        #                 'samples': { wjbin:  ['1.','1.']},
        #                 'folderUp' : directory_wjets_res_bins[wjbin]+'_fatjetJESup',
        #                 'folderDown' : directory_wjets_res_bins[wjbin]+'_fatjetJESdo',
        #                 'AsLnN'      : '1',  
        # }
...
nuisances['PS_ISR_QCD_WV']  = {
                    'name'  : 'CMS_PS_ISR_QCD_WV',
                    'kind'  : 'weight',
                    'type'  : 'shape',
                    'samples'  : {
                        sample : ['PSWeight[2]', 'PSWeight[0]'] for sample in VV_WV_samples
                    }
                }
nuisances['PS_FSR_QCD_WV']  = {
                'name'  : 'CMS_PS_FSR_QCD_WV',
                'kind'  : 'weight',
                'type'  : 'shape',
                'samples'  : {
                    sample :  ['PSWeight[3]', 'PSWeight[1]'] for sample in VV_WV_samples
                }
            }

nuisances['PS_ISR_QCD_ZV']  = {
                    'name'  : 'CMS_PS_ISR_QCD_ZV',
                    'kind'  : 'weight',
                    'type'  : 'shape',
                    'samples'  : {
                        sample : ['PSWeight[2]', 'PSWeight[0]'] for sample in VV_ZV_samples
                    }
                }
nuisances['PS_FSR_QCD_ZV']  = {
                'name'  : 'CMS_PS_FSR_QCD_ZV',
                'kind'  : 'weight',
                'type'  : 'shape',
                'samples'  : {
                    sample :  ['PSWeight[3]', 'PSWeight[1]'] for sample in VV_ZV_samples
                }
            }
...
nuisances['pdf_weight'] = {
    ...
    'type'  : 'shape',
    'samples' :  { s: [' Alt$(LHEPdfWeight['+str(i)+'], 1.)' for i in range(0,103)] for s in mc if s not in ["VBS","VBS_dipoleRecoil", "VV", "top","Wjets_boost"]+wjets_res_bins + VBS_samples},
    'AsLnN':  '1'
}
...
nuisances['UE']  = {
                ...
                'skipCMS' : 1,
                'type': 'lnN',
                'samples': dict((skey, '1.015') for skey in mc if not skey in ['top','Wjets_boost']+wjets_res_bins),
}
