...
#ifndef Wtagging_SF_H
#define Wtagging_SF_H
...
DoubleValueReader* Wtagging_SF::VBS_category{};
...
FloatArrayReader* Wtagging_SF::Lepton_pt{};
FloatArrayReader* Wtagging_SF::Lepton_eta{};
FloatArrayReader* Wtagging_SF::Lepton_phi{};
  ...
  //resolved category
    ...
    if (*(nFatJet->Get()) > 0){
      ...
      if (fatjet.Pt() > 200 && abs(fatjet.Eta())< 2.4 && fatjet.M() > 40 && fatjet.M() < 250){
        ...
        // Check the overlap with lepton
        if (lep.DeltaR(fatjet) > 0.8) {
