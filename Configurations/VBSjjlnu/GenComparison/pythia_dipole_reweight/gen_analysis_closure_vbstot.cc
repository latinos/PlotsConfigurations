...
std::vector<RResultPtr<TH1D>> get_histograms_1D( RNode df, string label){
    ...
    return {
        ...
        df.Histo1D({(label+ "_mjj_morebins").c_str(), "mjj", 60, 50, 2000}, "mjj","Wtot"),
        df.Histo1D({(label+ "_mjj_morebins_100").c_str(), "mjj", 60, 100, 3000}, "mjj","Wtot"),
        ...
        df.Histo1D({(label+ "_vbs1_pt").c_str(), "vbs1_pt", 50, 30, 300}, "vbs_1_pt","Wtot"),
...
//         df.Histo2D({(label+ "_vbstot_detajj").c_str(), "vbstot_detajj", 19,ptjet3, 8,0,8} , "vbs_tot_pt", "detajj","Wtot"),
...
void analyze_sample(string name, RNode df, TFile& output,vector<TF1*> weights_mjj,vector<float> minmjjs,vector<TF1*> weights_vbstot,vector<std::pair<float,float>> rangevbs){
    ...
    auto mjj100_cut = vars_incl_cleanlep.Filter("mjj>=100");
...
int main(int argc, char** argv){
    ...
    analyze_sample("pythia", rdfP, output_file, weights_mjj,minmjjs, weights_vbstot, rangevbs);
    ...
    analyze_sample("pythiaDipole", rdfPd, output_file,weights_mjj,minmjjs, weights_vbstot, rangevbs);
