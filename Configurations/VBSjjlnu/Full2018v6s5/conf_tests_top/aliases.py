...
aliases['bReqTight'] = {
    'expr': '( Sum$(CleanJet_pt > 30. && abs(CleanJet_eta) < 2.5 && Jet_btagDeepB[CleanJet_jetIdx] > 0.7527) >= 1) && \
             (Sum$(CleanJet_pt > 30. && abs(CleanJet_eta) < 2.5 && Jet_btagDeepB[CleanJet_jetIdx] > 0.1241) >= 2)'
}
...
for s in systs:
  aliases['btagSF'+s+'up'] = { 'expr': '(bVeto*'+aliases['bVetoSF']['expr'].replace('shape','shape_up_'+s)+'+ ( bReq || bReqTight)*'+aliases['bReqSF']['expr'].replace('shape','shape_up_'+s)+')', 'samples':mc  }
  aliases['btagSF'+s+'down'] = { 'expr': '(bVeto*'+aliases['bVetoSF']['expr'].replace('shape','shape_down_'+s)+'+ ( bReq || bReqTight)*'+aliases['bReqSF']['expr'].replace('shape','shape_down_'+s)+')', 'samples':mc  }
