...
class GenJetBin_resolved : public multidraw::TTreeFunction {
public:
  GenJetBin_resolved();

  char const* getName() const override { return "GenJetBin_resolved"; }
  TTreeFunction* clone() const override { return new GenJetBin_resolved(); }
...
GenJetBin_resolved::GenJetBin_resolved() :
  TTreeFunction()
{
}

double
GenJetBin_resolved::evaluate(unsigned)
{
  ...
  if (njet>=4){
    ...
    else if (vbs_1_pt>= 75 && vbs_1_pt < 150){
      if(detavbs < 4) return 3;
      else return 4;
    }
    else return 5;
...
void
GenJetBin_resolved::bindTree_(multidraw::FunctionLibrary& _library)
{
