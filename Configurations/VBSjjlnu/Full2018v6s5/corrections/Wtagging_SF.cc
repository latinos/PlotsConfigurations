...
public:
  Wtagging_SF( char const* _type);
  Wtagging_SF( unsigned type);

  char const* getName() const override { return "Wtagging_SF"; }
  TTreeFunction* clone() const override { return new Wtagging_SF(returnVar_); }
...
protected:
  enum ReturnType {
        nominal_2018,up_2018,down_2018,nVarTypes
  };
...
map<string,float> Wtagging_SF::sf_hp = { {"2018", 0.980 }};
map<string,float> Wtagging_SF::sf_hp_uncert = { {"2018", 0.027 }};
map<string,float> Wtagging_SF::sf_lp = { {"2018", 1.200 }};
map<string,float> Wtagging_SF::sf_lp_uncert =  { {"2018", 0.275 }};

std::array<double, Wtagging_SF::nVarTypes> Wtagging_SF::returnValues{};

Wtagging_SF::Wtagging_SF( char const* _type):
   TTreeFunction(){
     
    std::string type(_type);
    if (type ==  "nominal_2018")
      returnVar_ = nominal_2018;
    else if (type == "up_2018")
      returnVar_ = up_2018;
    else if (type == "down_2018")
      returnVar_ = down_2018;
    else
      throw std::runtime_error("unknown return type " + type);
}

Wtagging_SF::Wtagging_SF( unsigned type):
...
Wtagging_SF::setValues(UInt_t _run, UInt_t _luminosityBlock, ULong64_t _event)
{
  ...
  if (vbs_cat == 0){
    ...
    returnValues[nominal_2018] = sf_hp["2018"];
    returnValues[up_2018]      = sf_hp["2018"] + sf_hp_uncert["2018"];
    returnValues[down_2018]    = sf_hp["2018"] - sf_hp_uncert["2018"];
  }
  //resolved category
  else if (vbs_cat == 1){
    ...
    if (good_fatjet){
      returnValues[nominal_2018] = sf_lp["2018"];
      // Anticorrelate the weights with HP region
      returnValues[up_2018]      = sf_lp["2018"] - sf_lp_uncert["2018"];
      returnValues[down_2018]    = sf_lp["2018"] + sf_lp_uncert["2018"];
    }else{
      returnValues[nominal_2018] = 1.0;
      returnValues[up_2018]      = 1.0;
      returnValues[down_2018]    = 1.0;
    }
  }
