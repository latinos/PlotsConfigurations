...
#     'linesToAdd' : [
#         'gSystem->Load("libLatinoAnalysisMultiDraw.so")',
#         '.L {}/VBSjjlnu/macros/is_data.cc+'.format(configurations)
#     ]
# }

# aliases['w_had_pt_scaled'] = {
#     'expr': '(is_data)*w_had_pt + (!is_data)*((w_had_pt <= 300)*w_had_pt + (w_had_pt > 300)*(0.922*(w_had_pt-300) + 300) ) ',
# }

# aliases['mjj_vjet_scaled'] = {
#     'expr': '(is_data)*mjj_vjet + (!is_data)*((w_had_pt <= 300)*mjj_vjet + (w_had_pt > 300)*(0.922*mjj_vjet))',
# }

aliases['w_had_pt_weight'] =  {
    'class' : 'ReweightWhadPt',
    'args': ('{}/VBSjjlnu/weights_files/whad_pt_weights/reweights_whadpt_wjets.root'.format(configurations),'wf_norm'),
    'linesToAdd' : [
        'gSystem->Load("libLatinoAnalysisMultiDraw.so")',
        '.L {}/VBSjjlnu/macros/reweight_WhadPt.cc+'.format(configurations)
    ]
}
...
do_morph = "11111111"
m_gluon_loweta_pt0 = "j3_loweta_ptF0_gluon"
...
aliases['DNNoutput_boosted'] = {
    'class': 'MVAReaderBoosted_mVscan',
    'args': ( models_path +'boost_sig/models/v3_d/',  models_path +'boost_sig/models/v3_d/cumulative_signal_2018.root', 
            False, 0, 95, 95, 1),
    'linesToAdd':[
        'gSystem->Load("libLatinoAnalysisMultiDraw.so")',
        'gSystem->Load("libDNNEvaluator.so")',
        '.L ' + mva_reader_path + 'mva_reader_boosted_v3d_mVscan.cc+', 
    ],
}