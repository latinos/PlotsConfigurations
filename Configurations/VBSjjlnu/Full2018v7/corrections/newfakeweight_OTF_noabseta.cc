...
class newFakeWeightOTF_noabs : public multidraw::TTreeFunction {
public:
  //newFakeWeightOTF_noabs(string file, string ele_WP, string mu_WP, string fr_ele_path, string pr_ele_path, string fr_mu_path, string pr_mu_path);
  newFakeWeightOTF_noabs(string ele_WP, string mu_WP, string fr_ele_path, string pr_ele_path, string fr_mu_path, string pr_mu_path);

  char const* getName() const override { 
      return "newFakeWeightOTF_noabs"; 
  }
  TTreeFunction* clone() const override { 
      //return new newFakeWeightOTF_noabs(inputfile_path, ele_WP, mu_WP, fr_ele_path, pr_ele_path, fr_mu_path, pr_mu_path); 
      return new newFakeWeightOTF_noabs(ele_WP, mu_WP, fr_ele_path, pr_ele_path, fr_mu_path, pr_mu_path); 
  }
...
//newFakeWeightOTF_noabs::newFakeWeightOTF_noabs(string in_file, string ele_WP, string mu_WP, string in_fr_ele_path, string in_pr_ele_path, string in_fr_mu_path, string in_pr_mu_path) :
newFakeWeightOTF_noabs::newFakeWeightOTF_noabs(string ele_WP, string mu_WP, string in_fr_ele_path, string in_pr_ele_path, string in_fr_mu_path, string in_pr_mu_path) :
  TTreeFunction(), 
...
double
newFakeWeightOTF_noabs::evaluate(unsigned)
{
  ...
  if (abs(Lepton_pdgId->At(0)) != 11){
      ...
      if (pt > maxpt){
        fake_rate = fr_mu_h2->GetBinContent(fr_mu_h2->FindBin(maxpt, eta)); //----> moving to eta without abs
      }else{ 
        fake_rate = fr_mu_h2->GetBinContent(fr_mu_h2->FindBin(pt, eta)); //----> moving to eta without abs
      }
      ...
      if (pt > maxpt){
        fake_rate = fr_ele_h2->GetBinContent(fr_ele_h2->FindBin(maxpt, eta));//----> moving to eta without abs
      }else{
        fake_rate = fr_ele_h2->GetBinContent(fr_ele_h2->FindBin(pt, eta));//----> moving to eta without abs
      }
...
void
newFakeWeightOTF_noabs::bindTree_(multidraw::FunctionLibrary& _library)
{   
...
newFakeWeightOTF_noabs::~newFakeWeightOTF_noabs(){
  Lepton_pt=nullptr;
  Lepton_eta= nullptr;
