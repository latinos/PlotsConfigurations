...
mc =["DY", "top", "VV", "VVV", "VBF-V", "Vg", "VgS", "VBS", "ggWW","VBS_dipoleRecoil"] + wjets_all_bins
...
nuisances['fatjetJMS']  = {
    ...
    'mapDown': 'fatjetJMSdo',
    'cuts': phase_spaces_boost, #because we are vetoing fatjets anyway in resolved category
    'samples': dict((skey, ['1.','1.']) for skey in mc if skey not in ["Vg","VgS", "VV", "ggWW"]+wjets_all_bins),
...
# samples_PS = ['VBS_dipoleRecoil','top','DY','VV','VVV','Vg','VgS','VBF-V','ggWW'] + wjets_bins #+['VBS_dipoleRecoil']
# # #
# # # PS and UE
# # # #
...
nuisances['PU']  = {
                ...
                'samples'  : {
                    s : ['(puWeightUp/puWeight) * {}'.format(nuis_factors[s]["CMS_PU_2018"][0]),
                         '(puWeightDown/puWeight) * {}'.format(nuis_factors[s]["CMS_PU_2018"][1])] for s in mc if s not in wjets_all_bins},
                'AsLnN'      : '1',
}
...
nuisances['pdf_weight'] = {
    ...
    'type'  : 'shape',
    'samples' :  { s: [' Alt$(LHEPdfWeight['+str(i)+'], 1.)' for i in range(0,103)] for s in mc if s not in ["VBS", "top"]+wjets_all_bins},
    'AsLnN':  '1'
}
