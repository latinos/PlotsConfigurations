...
samples['WW'] = {
    ...
    'weight': mcCommonWeight + '*nllW*qq2vvOTF',
    'FilesPerJob': 1
}
...
addSampleWeight(samples, 'VgS', 'ZGToLLG', '((Gen_ZGstar_mass > 0)*(Sum$(GenPart_pdgId == 22 && TMath::Odd(GenPart_statusFlags) && GenPart_pt < 20.) == 0)*55.5*1.06/131.3)')
...
for sname, tname in [('WW', 'WWTo2L2Nu'), ('ggH_hww', 'GluGluHToWWTo2L2NuPowheg_M125'), ('qqH_hww', 'VBFHToWWTo2L2NuPowheg_M125')]:
    for vname, dsuffix in [('UE_CP5Up', 'UEup'), ('UE_CP5Down', 'UEdo')]:
