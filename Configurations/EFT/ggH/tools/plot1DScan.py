...
def BuildScan(scan, param, files, color, yvals, ycut):
    ...
    global NAMECOUNTER
    func = ROOT.TF1('splinefn'+str(NAMECOUNTER), partial(Eval, spline), graph.GetX()[0], graph.GetX()[graph.GetN() - 1], 1) #graph.GetX()[0] to graph.GetX()[graph.GetN() - 1] KELLO, change boundaries in case of sharp peaks
    NAMECOUNTER += 1
...
new_min = -1.0 #axishist.GetXaxis().GetXmin()
new_max = 1.0 #axishist.GetXaxis().GetXmax()

print("MAX X: "+str(new_max))
mins = []
maxs = []
...
#main_scan['func'].Draw('same') #try spline instead of func
for other in other_scans:
    if args.breakdown is not None:
...
#textfit = '%s = %.3f{}^{#plus %.3f}_{#minus %.3f}' % (fixed_name, val_nom[0], val_nom[1], abs(val_nom[2])) #KELLO disabled best fit text


pt = ROOT.TPaveText(0.59, 0.82 - len(other_scans)*0.08, 0.95, 0.91, 'NDCNB')
#pt.AddText(textfit) KELLO disabled best fit text
...
#save_graph.GetXaxis().SetTitle('%s = %.3f %+.3f/%+.3f' % (fixed_name, val_nom[0], val_nom[2], val_nom[1])) #KELLO disabled for H0M
