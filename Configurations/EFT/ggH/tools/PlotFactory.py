...
class PlotFactory:
    ...
    def makePlot(self, inputFile, outputDirPlots, variables, cuts, samples, plot, nuisances, legend, groupPlot):
            ...
            if len(groupPlot.keys()) == 0:          
              ...
              if len(sigSupList_grouped) != 0:
                for histo in sigSupList_grouped:
                  histo.SetLineStyle(groupPlot[sampleNameGroup]['style']) #KELLO override
	    ...
	    #     - then the superimposed MC
            ...
            #     - then the DATA  
            if tgrData.GetN() != 0:
              for ni in range(0,tgrData.GetN()):
                tgrData.SetPointEXhigh(ni,0.)
                tgrData.SetPointEXlow(ni,0.)
              tgrData.Draw("P0")
              ...
              for sampleName, plotdef in plot.iteritems():
                if 'samples' in variable and sampleName not in variable['samples']:
                  continue
                if plotdef['isData'] == 1 :
                  histos[sampleName].Draw("p X0 same")

            #---- the Legend
            #tlegend = ROOT.TLegend(0.20, 0.65, 0.80, 0.88)
            #tlegend = ROOT.TLegend(0.20, 0.65, 0.65, 0.88) #KELLO
            if self.isVBF or self.isVH:
                #tlegend = ROOT.TLegend(0.20, 0.7, 0.90, 0.88) #KELLO KELLO 
                tlegend = ROOT.TLegend(0.20, 0.65, 0.92, 0.86)
            elif self.isVHB or self.isGGH or self.isGGH2j:
                tlegend = ROOT.TLegend(0.20, 0.65, 0.92, 0.86) 
            ...
            if len(groupPlot.keys()) == 0:
              ...
              for sampleName in reversedSampleNames:
                ...
                if plotdef['isData'] == 1 :
                  if 'nameHR' in plotdef.keys() :
                    if self._showIntegralLegend == 0 :
                      tlegend.AddEntry(histos[sampleName], plotdef['nameHR'], "EP")
                    else :
                      if variable["divideByBinWidth"] == 1:
                        nevents = histos[sampleName].Integral(1,histos[sampleName].GetNbinsX(),"width")
                      else:
                        nevents = histos[sampleName].Integral(1,histos[sampleName].GetNbinsX())
                      tlegend.AddEntry(histos[sampleName], plotdef['nameHR'] + " [" +  str(round(nevents,1)) + "]", "EP")
                  else :
                    if self._showIntegralLegend == 0 :
                      tlegend.AddEntry(histos[sampleName], sampleName, "EP")
                    else :
                      if variable["divideByBinWidth"] == 1:
                        nevents = histos[sampleName].Integral(1,histos[sampleName].GetNbinsX(),"width")
                      else:
                        nevents = histos[sampleName].Integral(1,histos[sampleName].GetNbinsX())
                      tlegend.AddEntry(histos[sampleName], sampleName + " [" +  str(round(nevents,1)) + "]", "EP")
              ...
              groupPlotReversed = OrderedDict(reversed(list(groupPlot.items()))) #KELLO  
              for sampleNameGroup, sampleConfiguration in groupPlotReversed.iteritems(): #KELLO reversed
                if 'samples' in variable and len(set(sampleConfiguration['samples']) & set(variable['samples'])) == 0:
                  continue
            ...
            if not self._preliminary :
              CMS_lumi.extraText = ""
            if self._middleText is not None:
              CMS_lumi.writeMiddleText = True #KELLO
              CMS_lumi.middleText = self._middleText
              if (self.isVH or self.isVHB) and False: 
                  CMS_lumi.middleTextSize = 0.8
                  CMS_lumi.middleTextScaleX = 4.2
              elif self.isVH or self.isVHB: #only for H0PH, H0LZg and H0L1
                  CMS_lumi.middleTextScaleX = 3.9 
              if self.isGGH2j:
                  CMS_lumi.middleTextSize = 0.7
                  CMS_lumi.middleTextScaleX = 4.2
              if self.isVBF and False: #Enable only for H0L1 and H0LZg
                  CMS_lumi.middleTextScaleX = 3.2 
            ...
            pad1 = ROOT.TPad(canvasPad1Name,canvasPad1Name, 0, 1-0.70, 1, 1) #KELLO 1-0.72
            pad1.SetTopMargin(0.098) #KELLO 0.098
            pad1.SetBottomMargin(0.002) #KELLO 0.020 
            pad1.Draw()
            #pad1.cd().SetGrid()
            ...
            #xAxisDistro.SetNdivisions(6,5,0) #KELLO main plot bins
            xAxisDistro.SetNdivisions(5,10,0)
            ...
            if self._extraLegend is not None:
                ...
                legExtra.SetTextSize(0.047) #KELLO before 0.045
                legExtra.SetTextAngle(0)
                legExtra.SetTextAlign(12) #KELLO before 22
                legExtra.SetTextFont(22) #KELLO before 62
                if "@@" in self._extraLegend: self._extraLegend = self._extraLegend.split("@@")
                else: self._extraLegend = [self._extraLegend]
                dy = 0.06
                for iline,line in enumerate(self._extraLegend): 
                    print(line)
                    legExtra.DrawLatexNDC(0.68,0.82-dy*iline,line)
                #legExtra.DrawLatexNDC(0.85,0.7,"0^{-1}") 

	    #KELLO vertical lines for bin thresholds
            if self.isVBF: 
                thresh1 = ["D_{VBF} < 0.75","D_{VBF} > 0.75"]
                thresh2 = ["m_{ll} < 45 GeV","m_{ll} > 45 GeV"] 
                thresh3 = []
            elif self.isVH or self.isVHB:
                thresh1 = ["m_{ll} < 45 GeV","m_{ll} > 45 GeV"]
                thresh2 = []
                thresh3 = []   
            elif self.isGGH:
                thresh1 = []
                thresh2 = []
                thresh3 = ["#in ["+str(r[0])+","+str(r[1])+"]" for r in [(60,70),(70,80),(80,90),(90,100),(100,110),(110,125)]] 
                #thresh3 = ["m_{T} = 70 GeV","m_{T} = 80 GeV","m_{T} = 90 GeV","m_{T} = 100 GeV","m_{T} = 110 GeV"]
            elif self.isGGH2j:
                thresh1 = ["D_{VBF} < 0.5","D_{VBF} > 0.5"]
                thresh2 = []
                thresh3 = [] 
            #thresh1 = ["D_{VBF} = 0.5"] #VBF ggH+2j
            #thresh3 = ["m_{T}^{H} = 70 GeV","m_{T}^{H} = 80 GeV","m_{T}^{H} = 90 GeV","m_{T}^{H} = 100 GeV","m_{T}^{H} = 110 GeV"]
            binWidth = frameDistro.GetXaxis().GetBinWidth(1) #bins are equidistant
             
            #VBF/VH
            if self.isVBF: 
                x1 = frameDistro.GetXaxis().GetXmin()+binWidth*(frameDistro.GetXaxis().GetNbins()/2) #VBF AC and SMEFT INN
                #x1 = frameDistro.GetXaxis().GetXmin()+binWidth*6*(frameDistro.GetXaxis().GetNbins()/11) #VBF SMEFT INP
                #vertLine1 = ROOT.TLine(x1, min(0.001, minYused), x1, maxYused*0.65);
                vertLine1 = ROOT.TLine(x1,  0., x1, maxYused*0.75) 
            elif self.isVH or self.isVHB:
                x1 = frameDistro.GetXaxis().GetXmin()+binWidth*(frameDistro.GetXaxis().GetNbins()/2)
                vertLine1 = ROOT.TLine(x1,  0., x1, maxYused*0.70)
            if self.isVBF or self.isVH or self.isVHB: 
                vertLine1.SetLineStyle(1)
                vertLine1.SetLineWidth(4)
                vertLine1.Draw("SAME") 
                binLabel1a =  ROOT.TLatex()
                binLabel1b =  ROOT.TLatex()
                #binLabel1.SetTextAngle(90) #KELLO
                binLabel1a.SetTextAlign(12)
                binLabel1a.SetTextFont(42) #22
                binLabel1a.SetTextSize(0.05)
                binLabel1b.SetTextAlign(12)
                binLabel1b.SetTextFont(42) #22
                binLabel1b.SetTextSize(0.05)
            if self.isVBF:  
                #binLabel1.DrawLatexNDC(0.53,0.35,thresh1[0]) #VBF a2 B, a2 IN A, VH
                #binLabel1.DrawLatexNDC(0.59,0.35,thresh1[0])  #VBF a2 A
                binLabel1a.DrawLatexNDC(0.28,0.60,thresh1[0]) #VBF a3
                binLabel1b.DrawLatexNDC(0.68,0.60,thresh1[1])
                #binLabel1.DrawLatexNDC(0.53,0.33,thresh1[0]) #VBF/VH prefit
            elif self.isVH or self.isVHB:
                binLabel1a.DrawLatexNDC(0.28,0.55,thresh1[0])  
                binLabel1b.DrawLatexNDC(0.68,0.55,thresh1[1]) 
                #binLabel1.DrawLatexNDC(0.59,0.33,thresh1[0]) #VH Boosted IP a3, VH a2 prefit
                #binLabel1.DrawLatexNDC(0.59,0.35,thresh1[0]) #VH Boosted IP a3 prefit
            
            #ggH+2j
            if self.isGGH2j:
                x1 = frameDistro.GetXaxis().GetXmin()+binWidth*(5*frameDistro.GetXaxis().GetNbins()/13) 
                #vertLine1 = ROOT.TLine(x1, min(0.001, minYused), x1, maxYused*0.65);
                vertLine1 = ROOT.TLine(x1,  0., x1, maxYused*0.65); #ggH+2j
                vertLine1.SetLineStyle(1)
                vertLine1.SetLineWidth(4)
                vertLine1.Draw("SAME")
                ...
                binLabel1b.SetTextFont(42) #22
                binLabel1b.SetTextSize(0.05) 
                binLabel1a.DrawLatexNDC(0.25,0.55,thresh1[0])
                binLabel1b.DrawLatexNDC(0.65,0.55,thresh1[1])

            #VBF only
            if self.isVBF:
                x2 = frameDistro.GetXaxis().GetXmin()+binWidth*(frameDistro.GetXaxis().GetNbins()/4) #VBF AC and SMEFT INN
                #x2 = frameDistro.GetXaxis().GetXmin()+binWidth*3*(frameDistro.GetXaxis().GetNbins()/11) #VBF SMEFT INP
                vertLine2 = ROOT.TLine(x2, min(0.001, minYused), x2, maxYused*0.60);
                vertLine2.SetLineStyle(7)
                vertLine2.SetLineWidth(4)
                vertLine2.SetLineColor(ROOT.kBlue)
                vertLine2.Draw("SAME")
                binLabel2a =  ROOT.TLatex()
                binLabel2a.SetTextColor(ROOT.kBlue)
                binLabel2a.SetTextAlign(12)
                binLabel2a.SetTextFont(42) #22
                binLabel2a.SetTextSize(0.038)
                binLabel2a.DrawLatexNDC(0.20,0.45,thresh2[0]) #VBF postfit
                binLabel2b =  ROOT.TLatex()
                binLabel2b.SetTextColor(ROOT.kBlue) 
                binLabel2b.SetTextAlign(12)
                binLabel2b.SetTextFont(42) #22
                binLabel2b.SetTextSize(0.038)
                binLabel2b.DrawLatexNDC(0.40,0.45,thresh2[1]) #VBF postfit 
                #binLabel2.DrawLatexNDC(0.33,0.33,thresh2[0]) #VBF prefit

	        x3 = frameDistro.GetXaxis().GetXmin()+binWidth*(3*frameDistro.GetXaxis().GetNbins()/4)
                vertLine3 = ROOT.TLine(x3, min(0.001, minYused), x3, maxYused*0.60);
                vertLine3.SetLineStyle(7)
                vertLine3.SetLineWidth(4)
                vertLine3.SetLineColor(ROOT.kBlue)
                vertLine3.Draw("SAME")
                binLabel3a =  ROOT.TLatex()
                binLabel3a.SetTextColor(ROOT.kBlue)
                binLabel3a.SetTextAlign(12)
                binLabel3a.SetTextFont(42) #22
                binLabel3a.SetTextSize(0.038)
                #binLabel3a.DrawLatexNDC(0.60,0.45,thresh2[0]) #VBF postfit for font 22
                binLabel3a.DrawLatexNDC(0.59,0.45,thresh2[0])
                binLabel3b =  ROOT.TLatex()
                binLabel3b.SetTextColor(ROOT.kBlue)
                binLabel3b.SetTextAlign(12)
                binLabel3b.SetTextFont(42) #22
                binLabel3b.SetTextSize(0.038)
                #binLabel3b.DrawLatexNDC(0.80,0.45,thresh2[1]) #VBF postfit for font 22
                binLabel3b.DrawLatexNDC(0.79,0.45,thresh2[1]) 
                #binLabel3.DrawLatexNDC(0.79,0.33,thresh2[0]) #VBF prefit
            
            #ggF only
            if self.isGGH:
                x4 = frameDistro.GetXaxis().GetXmin()+binWidth*(1*frameDistro.GetXaxis().GetNbins()/6)
                vertLine4 = ROOT.TLine(x4, 0., x4, maxYused*0.65);
                vertLine4.SetLineStyle(7)
                vertLine4.SetLineWidth(4)
                vertLine4.Draw("SAME")

                x5 = frameDistro.GetXaxis().GetXmin()+binWidth*(2*frameDistro.GetXaxis().GetNbins()/6)
                vertLine5 = ROOT.TLine(x5, 0., x5, maxYused*0.65);
                vertLine5.SetLineStyle(7)
                vertLine5.SetLineWidth(4)
                vertLine5.Draw("SAME")

                x6 = frameDistro.GetXaxis().GetXmin()+binWidth*(3*frameDistro.GetXaxis().GetNbins()/6)
                vertLine6 = ROOT.TLine(x6, 0., x6, maxYused*0.65);
                vertLine6.SetLineStyle(7)
                vertLine6.SetLineWidth(4)
                vertLine6.Draw("SAME")

                x7 = frameDistro.GetXaxis().GetXmin()+binWidth*(4*frameDistro.GetXaxis().GetNbins()/6)
                vertLine7 = ROOT.TLine(x7, 0., x7, maxYused*0.65);
                vertLine7.SetLineStyle(7)
                vertLine7.SetLineWidth(4)
                vertLine7.Draw("SAME")

                x8 = frameDistro.GetXaxis().GetXmin()+binWidth*(5*frameDistro.GetXaxis().GetNbins()/6)
                vertLine8 = ROOT.TLine(x8, 0., x8, maxYused*0.65);
                vertLine8.SetLineStyle(7)
                vertLine8.SetLineWidth(4)
                vertLine8.Draw("SAME")   

                for i in range(1,7):
                    binLabel4 =  ROOT.TLatex()
                    binLabel4.SetTextAngle(0) #90
                    binLabel4.SetTextAlign(12)
                    binLabel4.SetTextFont(42) #22
                    binLabel4.SetTextSize(0.035)
                    #binLabel4.DrawLatexNDC(0.32+(i-1)*0.133,0.45,thresh3[i-1])
                    corr = 0 
                    if i == 4: corr = 0.006
                    if i > 4: corr = 0.011   
                    binLabel4.DrawLatexNDC(0.185+(i-1)*0.133-corr,0.57,thresh3[i-1])
                binLabel5 =  ROOT.TLatex()
                binLabel5.SetTextAngle(0) 
                binLabel5.SetTextAlign(12)
                binLabel5.SetTextFont(42)
                binLabel5.SetTextSize(0.035) 
                binLabel5.DrawLatexNDC(0.192,0.62,"m_{T} [GeV]") 
            ...
            pad2 = ROOT.TPad(canvasPad2Name,canvasPad2Name,0,0,1,1-0.70) #KELLO 1-0.72
            pad2.SetTopMargin(0.14) #KELLO 0.06
            pad2.SetBottomMargin(0.450) #KELLO 0.392
            pad2.Draw("SAME")
            pad2.cd()
            ...
            xAxisDistro.SetNdivisions(510)

            if 'xaxis' in variable.keys() : 
              frameRatio.GetXaxis().SetTitle(variable['xaxis'])
            else :
              frameRatio.GetXaxis().SetTitle(variableName)
            frameRatio.GetYaxis().SetTitle("Data/Expected") #KELLO #Type1

            #frameRatio.GetYaxis().SetTitle("#frac{Data}{SMHiggs+Bkg}") #Type2 for pre-fit and B-only postfit
            #frameRatio.GetYaxis().SetTitle("#frac{Data}{Tot.Sig+Bkg}") #Type3 for S+B post-fit
            #frameRatio.GetYaxis().SetTitle("Data/MC")
            #frameRatio.GetYaxis().SetRangeUser( 0.0, 2.0 )
            #frameRatio.GetYaxis().SetRangeUser( 0.55, 1.45 ) #origo
            #frameRatio.GetYaxis().SetRangeUser( 0.8, 1.2 ) #for pre-fit
            if (self.isVH or self.isVHB or self.isGGH) and True:
                frameRatio.GetYaxis().SetRangeUser( 0.5, 1.5 ) #for prefit unblinding
            if self.isVBF or self.isGGH2j: 
                frameRatio.GetYaxis().SetRangeUser( 0.0, 2.0 ) #for unblinding
            self.Pad2TAxis(frameRatio) #here style of labels and bin division is defined

            #KELLO change bin labels !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            print("Number of Bins = "+str(frameRatio.GetXaxis().GetNbins()))
            if self.isVBF:
                denom = 40. 
                step = 0.1
                angle = 90
                ...
                denom = 54.
                step = ["    12-17","    17-25","    25-30","    30-35","    35-40","    40-45","    45-65","  65-100","100-200"]
                angle = 90
                align = 23
                size = 0.06
                repeat = 9
                ...
                denom = 13.
                step = ["0.0-0.3","0.3-0.4","0.4-0.5","0.5-0.6","0.6-1.0","0.0-0.2","0.2-0.3","0.3-0.4","0.4-0.5","0.5-0.6","0.6-0.7","0.7-0.8","0.8-1.0"]
                angle = 40
                align = 12
                size = 0.09
                repeat = 13
            delta = int(frameRatio.GetXaxis().GetNbins()/denom)
            frameRatio.GetXaxis().SetAlphanumeric()
            frameRatio.GetXaxis().SetNdivisions(int(denom),0,0)
            for ibin in range(0,int(denom)+1):
                _ibin = ibin%repeat
                if self.isVBF: 
                    if(ibin==int(denom)):
                        frameRatio.GetXaxis().ChangeLabel(ibin+1,angle,size,32,-1,-1," ") 
                    else:
                        frameRatio.GetXaxis().ChangeLabel(ibin+1,angle,size,align,-1,-1,""+str(_ibin*step)+"-"+str((_ibin+1)*step)+"")
                if self.isVH:
                    if(ibin==int(denom)):
                        frameRatio.GetXaxis().ChangeLabel(ibin+1,angle,size,32,-1,-1," ")
                    else:
                        frameRatio.GetXaxis().SetLabelOffset(0.15)
                        frameRatio.GetXaxis().ChangeLabel(ibin+1,angle,size,align,-1,-1,""+str(_ibin*step)+"-"+str((_ibin+1)*step)+"")
                if self.isVHB:
                    if(ibin==int(denom)):
                        frameRatio.GetXaxis().ChangeLabel(ibin+1,angle,size,32,-1,-1," ") 
                        ...
                        frameRatio.GetXaxis().SetLabelOffset(0.15)
                        frameRatio.GetXaxis().ChangeLabel(ibin+1,angle,size,align,-1,-1,""+step[_ibin]+"")
                if self.isGGH:
                    if(ibin==int(denom)):
                        frameRatio.GetXaxis().ChangeLabel(ibin+1,angle,size,32,-1,-1," ")
                    else:
            ...
            #KELLO 
            for ni in range(0,tgrDataOverMC.GetN()):
                tgrDataOverMC.SetPointEXhigh(ni,0.)
                tgrDataOverMC.SetPointEXlow(ni,0.) 
            ...
            pad2.RedrawAxis()
            #pad2.SetGrid() #enable vertical and horizontal lines in ratio pad
            
            #VBF KELLO add grid vert by hand
            if self.isVBF:
                halfWay = (frameRatio.GetXaxis().GetXmin()+frameRatio.GetXaxis().GetXmax())/2
                qWay = (frameRatio.GetXaxis().GetXmin()+frameRatio.GetXaxis().GetXmax())/4
                q3Way = 3*qWay
                vertLineA = ROOT.TLine(halfWay, 0,  halfWay, 2);
                vertLineA.SetLineStyle(1)
                vertLineA.SetLineColor(ROOT.kBlack)
                vertLineA.SetLineWidth(1)
                vertLineA.Draw("same")
                
                vertLineB = ROOT.TLine(qWay, 0,  qWay, 2);
                vertLineB.SetLineStyle(2)
                vertLineB.SetLineColor(ROOT.kBlue)
                vertLineB.SetLineWidth(1)
                vertLineB.Draw("same") 
 
                vertLineC = ROOT.TLine(q3Way, 0,  q3Way, 2);
                vertLineC.SetLineStyle(2)
                vertLineC.SetLineColor(ROOT.kBlue)
                vertLineC.SetLineWidth(1)
                vertLineC.Draw("same")
            
            #VH KELLO add grid vert by hand
            if self.isVH or self.isVHB:
                qWay = (frameRatio.GetXaxis().GetXmin()+frameRatio.GetXaxis().GetXmax())/2
                vertLineB = ROOT.TLine(qWay, 0.5,  qWay, 1.5); 
                vertLineB.SetLineStyle(2)
                vertLineB.SetLineColor(ROOT.kBlack)
                vertLineB.SetLineWidth(1)
                vertLineB.Draw("same")

            #ggH 0/1-jet grid vert by hand
            if self.isGGH:
                qWay1 = 1*(frameRatio.GetXaxis().GetXmin()+frameRatio.GetXaxis().GetXmax())/6
                qWay2 = 2*(frameRatio.GetXaxis().GetXmin()+frameRatio.GetXaxis().GetXmax())/6 
                qWay3 = 3*(frameRatio.GetXaxis().GetXmin()+frameRatio.GetXaxis().GetXmax())/6
                qWay4 = 4*(frameRatio.GetXaxis().GetXmin()+frameRatio.GetXaxis().GetXmax())/6
                qWay5 = 5*(frameRatio.GetXaxis().GetXmin()+frameRatio.GetXaxis().GetXmax())/6
                vertLineB1 = ROOT.TLine(qWay1, 0.5,  qWay1, 1.5);
                vertLineB1.SetLineStyle(2)
                vertLineB1.SetLineColor(ROOT.kBlack)
                vertLineB1.SetLineWidth(1)
                vertLineB1.Draw("same")
 
                vertLineB2 = ROOT.TLine(qWay2, 0.5,  qWay2, 1.5);
                vertLineB2.SetLineStyle(2)
                vertLineB2.SetLineColor(ROOT.kBlack)
                vertLineB2.SetLineWidth(1)
                vertLineB2.Draw("same")

                vertLineB3 = ROOT.TLine(qWay3, 0.5,  qWay3, 1.5);
                vertLineB3.SetLineStyle(2)
                vertLineB3.SetLineColor(ROOT.kBlack)
                vertLineB3.SetLineWidth(1)
                vertLineB3.Draw("same")

                vertLineB4 = ROOT.TLine(qWay4, 0.5,  qWay4, 1.5);
                vertLineB4.SetLineStyle(2)
                vertLineB4.SetLineColor(ROOT.kBlack)
                vertLineB4.SetLineWidth(1)
                vertLineB4.Draw("same") 

                vertLineB5 = ROOT.TLine(qWay5, 0.5,  qWay5, 1.5);
                vertLineB5.SetLineStyle(2)
                vertLineB5.SetLineColor(ROOT.kBlack)
                vertLineB5.SetLineWidth(1)
                vertLineB5.Draw("same")

            #ggH +2j add grid vert by hand
            if self.isGGH2j:
                qWay = 5*(frameRatio.GetXaxis().GetXmin()+frameRatio.GetXaxis().GetXmax())/13
                vertLineB = ROOT.TLine(qWay, 0.0,  qWay, 2.0);
                vertLineB.SetLineStyle(2)
                vertLineB.SetLineColor(ROOT.kBlack)
                vertLineB.SetLineWidth(1)
                vertLineB.Draw("same")

            #VBF KELLO add grid horiz by hand
            if self.isVBF or self.isGGH2j:
                horLineA = ROOT.TLine(frameRatio.GetXaxis().GetXmin(), 0.5,  frameRatio.GetXaxis().GetXmax(), 0.5);
                horLineA.SetLineStyle(3)
                horLineA.SetLineColor(ROOT.kBlack)
                horLineA.SetLineWidth(1)
                horLineA.Draw("same")

                horLineB = ROOT.TLine(frameRatio.GetXaxis().GetXmin(), 1.5,  frameRatio.GetXaxis().GetXmax(), 1.5);
                horLineB.SetLineStyle(3)
                horLineB.SetLineColor(ROOT.kBlack)
                horLineB.SetLineWidth(1)
                horLineB.Draw("same")

            #VH KELLO add grid horiz by hand 
            if (self.isVH or self.isVHB or self.isGGH) and True:
                horLineA = ROOT.TLine(frameRatio.GetXaxis().GetXmin(), 0.6,  frameRatio.GetXaxis().GetXmax(), 0.6);
                horLineA.SetLineStyle(3)
                horLineA.SetLineColor(ROOT.kBlack)
                horLineA.SetLineWidth(1)
                horLineA.Draw("same")

                horLineB = ROOT.TLine(frameRatio.GetXaxis().GetXmin(), 0.8,  frameRatio.GetXaxis().GetXmax(), 0.8);
                horLineB.SetLineStyle(3)
                horLineB.SetLineColor(ROOT.kBlack)
                horLineB.SetLineWidth(1)
                horLineB.Draw("same")

                horLineC = ROOT.TLine(frameRatio.GetXaxis().GetXmin(), 1.2,  frameRatio.GetXaxis().GetXmax(), 1.2);
                horLineC.SetLineStyle(3)
                horLineC.SetLineColor(ROOT.kBlack)
                horLineC.SetLineWidth(1)
                horLineC.Draw("same")

                horLineD = ROOT.TLine(frameRatio.GetXaxis().GetXmin(), 1.4,  frameRatio.GetXaxis().GetXmax(), 1.4);
                horLineD.SetLineStyle(3)
                horLineD.SetLineColor(ROOT.kBlack)
                horLineD.SetLineWidth(1)
                horLineD.Draw("same")            
            ...
            if 'cratio' in self._plotsToWrite:
                ...
                if self._plotLog:
                    ...
                    if maxYused>0:
                      #KELLO
                      if self.isGGH:
                          frameDistro.GetYaxis().SetRangeUser( min(self._minLogCratio, maxYused/100), self._maxLogCratio * maxYused )
                      else:    
            ...
            # style from https://ghm.web.cern.ch/ghm/plots/MacroExample/myMacro.py
            xAxisDistro = frameDistro.GetXaxis()
            xAxisDistro.SetNdivisions(6,5,0) #KELLO
            ...
            if self._plotFancy and not self._showRelativeRatio:
              ...
              xaxis.SetTitleFont ( 22) #KELLO 42
              xaxis.SetTitleOffset( 1.35) #KELLO 1.35   
              xaxis.SetTitleSize ( 0.035) #KELLO 0.035
              ...
              yaxis.SetTitleFont ( 42) #KELLO 42 
              yaxis.SetTitleOffset( 1.55)
              yaxis.SetTitleSize ( 0.045) #KELLO 0.045
                ...
                if 'binX' in variable.keys() and 'binY' in variable.keys() :
                  ...
                  if thsBackground.GetNhists() != 0 and thsSignal.GetNhists() != 0 :
                    ...
                    for sliceX in range(nbinX) :
                      ...
                      if len(groupPlot.keys()) == 0:          
                          ...
                          for ihisto in range(thsSignal_grouped.GetNhists()):
                             ...
                             histo.SetFillStyle( hentry.GetFillStyle())
                             histo.SetLineColor( hentry.GetLineColor())
                             histo.SetLineStyle( plot[sampleName]['style'] ) #KELLO
                             histo.SetLineWidth( hentry.GetLineWidth())
                    ...
                    if weight_X_tgrData.GetN() != 0:
                      for ni in range(0,weight_X_tgrData.GetN()):
                          weight_X_tgrData.SetPointEXhigh(ni,0.)
                          weight_X_tgrData.SetPointEXlow(ni,0.)
              ...
              if self._plotNormalizedIncludeData : 
                for sampleName, plotdef in plot.iteritems():
                  if plotdef['isData'] == 1 :
                    ...
                    histos[sampleName].DrawNormalized("p X0, same")
   ...
   # --- squared sum
    def Pad2TAxis(self, hist):
         xaxis = hist.GetXaxis()
         xaxis.SetLabelFont ( 42)
         xaxis.SetLabelOffset( 0.055) #KELLO 0.025
         xaxis.SetLabelSize ( 0.1)
         xaxis.SetNdivisions (5,10,0) #KELLO 505
         xaxis.SetTitleFont ( 42) # KELLO 42
         xaxis.SetTitleOffset( 1.75) # KELLO 1.35  
         xaxis.SetTitleSize ( 0.12) # KELLO 0.11
         ...
         yaxis.SetTitleSize ( 0.11) #KELLO 0.11
