...
#inputFilePostfit = "output_postFitShapes_Run2_2jVBF_afterVgBug_narrowFai_H0LZg_fitS.root"
inputFilePostfit = "output_postFitShapes_Run2_VHBoosted_afterVgBug_narrowFai_H0LZg_fitS.root"
...
for h in histos:
    ...
    prenorm = 1.0
    if any(sub in h.GetName() for sub in ["VBF","ZH"]) and "T3" in h.GetName():
        prenorm = 0.99*0.01#0.0001
        h.Scale(prenorm)
        normBSM += h.Integral()
        for proc in onlyFor:
            if proc in h.GetName():
                normBSMOnlyFor += h.Integral() 
    elif any(sub in h.GetName() for sub in ["VBF","ZH"]) and "T2" in h.GetName():
        prenorm = (0.01**(3/2))*(0.99**(1/2))#0.01
        h.Scale(prenorm)
        normBSM += h.Integral()
        for proc in onlyFor:
            if proc in h.GetName():
...
for h in histos:
    ...
    if (any(sub in h.GetName() for sub in ["VBF","ZH"]) and ("T3" in h.GetName() or "T2" in h.GetName())):
        if scaleToPostfit:
