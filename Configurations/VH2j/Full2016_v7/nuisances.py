...
#  for cat in cuts[k]['categories']:
    ...
    else: print 'WARNING: name of cut does not contain on either 0j,1j,2j'
...
for shift in ['jes', 'lf', 'hf', 'hfstats1', 'hfstats2', 'lfstats1', 'lfstats2', 'cferr1', 'cferr2']:
    ...
    nuisances['btag_shape_%s' % shift] = {
        ...
        'kind': 'weight',
        'type': 'shape',
        'samples': dict((skey, btag_syst) for skey in mc if skey not in ['Vg']),
    }
...
nuisances['trigg'] = {
    ...
    'kind': 'weight',
    'type': 'shape',
    'samples': dict((skey, trig_syst) for skey in mc_emb if skey not in ['Vg'])
}
...
nuisances['prefire'] = {
    ...
    'kind': 'weight',
    'type': 'shape',
    'samples': dict((skey, prefire_syst) for skey in mc if skey not in ['Vg'])
}
...
nuisances['eff_e'] = {
    ...
    'kind': 'weight',
    'type': 'shape',
    'samples': dict((skey, ['SFweightEleUp', 'SFweightEleDown']) for skey in mc_emb if skey not in ['Vg'])
}
...
nuisances['electronpt'] = {
    ...
    'mapUp' : 'ElepTup',
    'mapDown': 'ElepTdo',
    'samples': dict((skey, ['1', '1']) for skey in mc if skey not in ['Vg']),
...
nuisances['eff_m'] = {
    ...
    'kind': 'weight',
    'type': 'shape',
    'samples': dict((skey, ['SFweightMuUp', 'SFweightMuDown']) for skey in mc_emb if skey not in ['Vg']),
}
...
nuisances['jetPUID'] = {
    ...
    'kind': 'weight',
    'type': 'shape',
    'samples': dict((skey, puid_syst) for skey in mc if skey not in ['Vg'])
}
...
nuisances['UE']  = {
                ...
                'skipCMS' : 1,
                'type': 'lnN',
                'samples': dict((skey, '1.015') for skey in mc if not skey in ['WW','top','Vg']),
}
