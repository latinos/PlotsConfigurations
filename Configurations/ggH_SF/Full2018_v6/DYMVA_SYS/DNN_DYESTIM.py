...
thefile = TFile('rootFile/plots_DYMVA_SYS_2018_v6.root')
...
#      histos['A_H_DATA_'+dirs.GetName()] = TH1D('A_H_DATA_'+dirs.GetName(), 'A_H_DATA_'+dirs.GetName(), histos['DATA_'+dirs.GetName()].GetNbinsX(), histos['DATA_'+dirs.GetName()].GetXaxis().GetXbins().GetArray())
  ...
  for subdir in currentdir.GetListOfKeys(): 
    if subdir.GetName()!='dymva_alt_dnn_0j': 
      continue
    ...
    for sample in currentsubdir.GetListOfKeys():
      ...
      if 'histo_DATA' in sample.GetName() or 'histo_DY' in sample.GetName():
        continue
      histos[sample.GetName()+dirs.GetName()] = thefile.Get(dirs.GetName()+'/dymva_alt_dnn_0j/'+sample.GetName())
...
#Fillin R histos
for dirs in thelist :
  if 'out' in dirs.GetName() and not 'df' in dirs.GetName() and not 'H_' in dirs.GetName() and not 'ww_' in dirs.GetName() and not '1j' in dirs.GetName() and not '2j' in dirs.GetName() and not 'VBF' in dirs.GetName() and not 'VH' in dirs.GetName():
    print(dirs.GetName())
    if 'btag' not in dirs.GetName():
    ...
    if 'out' in dirs.GetName() and not 'df' in dirs.GetName() and not 'H_' in dirs.GetName() and not 'ww_' in dirs.GetName() and 'btag' not in dirs.GetName() and not '1j' in dirs.GetName() and not '2j' in dirs.GetName() and not 'VBF' in dirs.GetName() and not 'VH' in dirs.GetName():
...
for histo in histos:
  if histo.startswith('R_') or histo.startswith('k_') or histo.startswith('A_') :
    histos[histo].Write()
  #Saving R histos
  if 'R_MC' in histo and 'btag' not in histo  and not '1j' in histo and not '2j' in histo and not 'VBF' in histo and not 'VH' in histo:
    print(histo)
    canvas = loadcanvas()
    canvas.cd()
  ...
  #Saving k histos
  if 'k_MC' in histo and 'btag' not in histo and not '1j' in histo and not '2j' in histo and not 'VBF' in histo and not 'VH' in histo:
    print(histo)
    canvas = loadcanvas()
    canvas.cd()
    legend = loadlegend(canvas.GetTopMargin(), canvas.GetBottomMargin(), canvas.GetLeftMargin(), canvas.GetRightMargin())
...
for graph in graphs:
  if 'MC' in graph and not '1j' in graph and not '2j' in graph and not 'VBF' in graph and not 'VH' in graph:
    print(graph)
    canvas = loadcanvas()
    canvas.cd()
    legend = loadlegend(canvas.GetTopMargin(), canvas.GetBottomMargin(), canvas.GetLeftMargin(), canvas.GetRightMargin())
