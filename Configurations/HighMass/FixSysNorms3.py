...
if not GetDone:
 for roo in rootdict:
  ...
  for r in relevant:
    allnuis[roo][r] = {}
  newfile = ROOT.TFile(rootdict[roo].replace(".root", "_new4.root"), 'read')
 ...
 with open('FixSysNorms_FactorsTEMPDELME3.json','w') as f:
  json.dump(allnuis, f)

else:
 with open('FixSysNorms_FactorsTEMPDELME3.json') as f:
  allnuis = json.load(f)

if False: # Here: Don't skip nuisances that aren't exclusive to the processes
   ...
   for categ in categs:
    ...
    for y in years:
      for c in allnuis[y][r]:
        ...
        if s+"Up" not in allnuis[y][r][c]: continue # Check if nuis of wrong category
        print "=========="
        print y,",",r,",",c,",",s
        print allnuis[y][r][c]
     ...
     for categ in categs:
      ...
      for y in years:
        for c in allnuis[y][r]:
          ...
          if "2018" in ts and ("2017" in y or "2016" in y): continue
          if "2018" in y and ("2017" in ts or "2016" in ts): continue
          if s+"Up" not in allnuis[y][r][c]: continue # Check if nuis of wrong category # Check if DYSF nuis
          UP += allnuis[y][r][c][ts+"Up"]
          NOM += allnuis[y][r][c]["nom"]
          DOWN += allnuis[y][r][c][ts+"Down"]
      if NOM==0.0: continue
...
print scaleby
with open('FixSysNorms_Factors3.json','w') as f:
  json.dump(scaleby, f)
