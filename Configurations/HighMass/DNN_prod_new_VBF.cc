...
class DNNprod_new_VBF : public multidraw::TTreeFunction {
public:
  DNNprod_new_VBF();

  char const* getName() const override { return "DNNprod_new_VBF"; }
  TTreeFunction* clone() const override { return new DNNprod_new_VBF(); }
...
DNNprod_new_VBF::DNNprod_new_VBF() :
  TTreeFunction()
{
  std::string cmsswbase(gSystem->Getenv("CMSSW_BASE"));
  dnn_tensorflow0 = new DNNEvaluator(cmsswbase + "/src/PlotsConfigurations/Configurations/HighMass/DNNs/ProdNew_0/", false, 1);
  dnn_tensorflow1 = new DNNEvaluator(cmsswbase + "/src/PlotsConfigurations/Configurations/HighMass/DNNs/ProdNew_1/", false, 1);
}

double
DNNprod_new_VBF::evaluate(unsigned)
{
...
void
DNNprod_new_VBF::bindTree_(multidraw::FunctionLibrary& _library)
{
