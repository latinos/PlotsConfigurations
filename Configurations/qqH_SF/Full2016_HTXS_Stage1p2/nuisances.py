...
for shift in ['jes', 'lf', 'hf', 'hfstats1', 'hfstats2', 'lfstats1', 'lfstats2', 'cferr1', 'cferr2']:
    ...
    nuisances['btag_shape_%s' % shift] = {
        ...
        'kind': 'weight',
        'type': 'shape',
        'samples': dict((skey, btag_syst) for skey in mc if skey not in ['DY']), # do we want to exclude DY?
    }
...
nuisances['trigg'] = {
    ...
    'kind': 'weight',
    'type': 'shape',
    'samples': dict((skey, trig_syst) for skey in mc if skey not in ['DY']) #['WW', 'top', 'DY'])
}
...
nuisances['prefire'] = {
    ...
    'kind': 'weight',
    'type': 'shape',
    'samples': dict((skey, prefire_syst) for skey in mc if skey not in ['DY']) #['WW', 'top', 'DY'])
}
...
nuisances['eff_m'] = {
    ...
    'kind': 'weight',
    'type': 'shape',
    'samples': dict((skey, ['ttHMVA_2l_mu_SF_Up', 'ttHMVA_2l_mu_SF_Down']) for skey in mc if skey not in ['DY']) #['WW', 'top', 'DY'])
}
...
for js in jes_systs:
  nuisances[js] = {
      ...
      'mapUp': js+'up',
      'mapDown': js+'do',
      'samples': dict((skey, ['1', '1']) for skey in mc if skey not in ['VZ','Vg','VgS', 'WW', 'top', 'DY', 'ggWW', 'WWewk']+signals),
