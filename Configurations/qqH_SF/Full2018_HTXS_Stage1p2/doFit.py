...
#No merging
command="combine -M MultiDimFit --algo=singles --X-rtd MINIMIZER_analytic Full2018_SF_qqH_HTXS_Stage1p2.root -t -1 --setParameters "
poi = ''
...
'''
#Merge some bins
command="combine -M MultiDimFit --algo=singles --X-rtd MINIMIZER_analytic Full2018_SF_ggH_HTXS_Stage1p2_merged.root -t -1 --setParameters "
poi = ''
for sample in sampleNames:
  if 'ggH_hww' not in sample: continue
  if 'FWDH' in sample: continue
...
#Merge all bins
command="combine -M MultiDimFit --algo=singles --X-rtd MINIMIZER_analytic Full2018_SF_ggH_HTXS_Stage1p2_onePOI.root -t -1 --setParameters "
poi = ''
for sample in sampleNames:
  if 'FWDH' in sample: continue
  else: poi = 'r_ggH_hww'
