...
genJetClean = 'TMath::Power(std_vector_jetGen_eta - std_vector_dressedLeptonGen_eta[0], 2.) + TMath::Power(TVector2::Phi_mpi_pi(std_vector_jetGen_phi - std_vector_dressedLeptonGen_phi[0]), 2.) > 0.16 && TMath::Power(std_vector_jetGen_eta - std_vector_dressedLeptonGen_eta[1], 2.) + TMath::Power(TVector2::Phi_mpi_pi(std_vector_jetGen_phi - std_vector_dressedLeptonGen_phi[1]), 2.) > 0.16'
nCleanGenJet = 'Sum$(std_vector_jetGen_pt > 30 && ' + genJetClean + ') == 0'
fid = 'std_vector_dressedLeptonGen_pt[0] > 25 && std_vector_dressedLeptonGen_pt[1] > 25 && fabs(std_vector_dressedLeptonGen_eta[0]) < 2.5 && fabs(std_vector_dressedLeptonGen_eta[1]) < 2.5 && abs(std_vector_leptonGen_MotherPID[0]) != 15 && abs(std_vector_leptonGen_MotherPID[1]) != 15 && (std_vector_leptonGen_pid[0] * std_vector_leptonGen_pid[1]== -11*11 || std_vector_leptonGen_pid[0] * std_vector_leptonGen_pid[1]== -13*13) && ' + gen_mllDressed + ' > 20. && ' + nCleanGenJet + ' && metGenpt > 20. && gen_ptll > 30.'
...
samples['WW']  = {    'name'   : getSampleFiles(directory,'WWTo2L2Nu') ,
                      ...
                      'linesToAdd' : ['.L /afs/cern.ch/work/f/fernanpe/framework/CMSSW_9_4_9/src/PlotsConfigurations/Configurations/WW/Full2016/onTheFly/addEWKcorr.C+'],  
                      'suppressNegative':['all'],
                      'suppressNegativeNuisances' :['all'],
                 }
