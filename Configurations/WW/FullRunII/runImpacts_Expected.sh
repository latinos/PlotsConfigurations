...
for year in "${YEARS[@]}"; do
  for jet in "${JETS[@]}"; do
    ...
    combineTool.py -M Impacts -d "../../${tag}.root" -m 125 -t -1 \
      --doInitialFit --expectSignal 1 --robustFit 1 --saveFitResult | tee "${tag}_Expected_SS.out"

    combineTool.py -M Impacts -d "../../${tag}.root" -m 125 -t -1\
      --expectSignal 1 --doFits --job-mode condor --task-name impacts --sub-opts '+JobFlavour = "longlunch"'

#    combineTool.py -M Impacts -d "../../${tag}.root" -m 125 -t -1 \
#      --expectSignal 1 --task-name impacts -o "impacts_${tag}_Expected.json"
