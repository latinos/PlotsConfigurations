...
command="text2workspace.py Full2018_ggH_HTXS_Stage1p2.txt -o Full2018_ggH_HTXS_Stage1p2_01J.root -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel --PO verbose "
for sample in sampleNames:
  if 'ggH_hww' not in sample: continue
  if 'FWDH' in sample: continue
  if 'GT200' in sample: continue
...
#Merge all bins
command="text2workspace.py Full2018_ggH_HTXS_Stage1p2.txt -o Full2018_ggH_HTXS_Stage1p2_onePOI.root -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel --PO verbose "
poi=''
for sample in sampleNames:
  if 'FWDH' in sample: continue
  else: poi ='r_ggH_hww'
