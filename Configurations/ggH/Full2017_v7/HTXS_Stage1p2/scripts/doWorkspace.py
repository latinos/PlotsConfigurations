...
##No merging
#command="text2workspace.py Full2017_ggH_HTXS_Stage2.txt -o Full2017_ggH_HTXS_Stage2.root -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel --PO verbose "
#for sample in sampleNames:
#  if 'ggH_hww' not in sample: continue
#  if 'FWDH' in sample: continue
#  if 'GT200' in sample: continue
#  if '0J' in sample or '1J' in sample:
...
#Merge 0J bins and 1J bins
command="text2workspace.py Full2017_ggH_HTXS_Stage2.txt -o Full2017_ggH_HTXS_Stage2_01J.root -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel --PO verbose "
for sample in sampleNames:
  if 'ggH_hww' not in sample: continue
  if 'FWDH' in sample: continue
  if 'GT200' in sample: continue
  if '0J' in sample:
    command+="--PO 'map=.*/{}:r_{}[1,-10,10]' ".format(sample,'ggH_hww_0J')
  elif '1J' in sample:
    command+="--PO 'map=.*/{}:r_{}[1,-10,10]' ".format(sample,'ggH_hww_1J')
  else: continue
...
#Merge all bins
command="text2workspace.py Full2017_ggH_HTXS_Stage2.txt -o Full2017_ggH_HTXS_Stage2_onePOI.root -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel --PO verbose "
poi=''
for sample in sampleNames:
  if 'FWDH' in sample: continue
  else: poi ='r_ggH_hww'
...
'''
#Merge some bins
command="text2workspace.py Full2017_ggH_HTXS_Stage2.txt -o Full2017_ggH_HTXS_Stage2_merged.root -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel --PO verbose "
poi=''
for sample in sampleNames:
  if 'ggH_hww' not in sample: continue
  if 'FWDH' in sample: continue
