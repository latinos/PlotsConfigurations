...
file = open("fitresults2018_thunorm.txt","r")
categories = []
...
for i,cat in enumerate(categories):
  ...
  graph.SetPoint(i,cat["value"],nChan-i)
  graph.SetPointError(i,cat["err_do"],cat["err_up"],0,0)


graph.GetXaxis().SetLimits(-5,6)
graph.GetYaxis().SetRangeUser(0,nChan+1)
...
graph.GetYaxis().SetNdivisions(nChan+1)
...
gPad.RedrawAxis()
canvas.Update()
canvas.SaveAs("signalstrength2018_1p2_thu.png")
a = raw_input()
