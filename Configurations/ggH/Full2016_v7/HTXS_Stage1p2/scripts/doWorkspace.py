...
##No merging
#command="text2workspace.py Full2016_ggH_HTXS_Stage2.txt -o Full2016_ggH_HTXS_Stage2.root -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel --PO verbose "
...
command="text2workspace.py Full2016_ggH_HTXS_Stage2.txt -o Full2016_ggH_HTXS_Stage2_01J.root -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel --PO verbose "
for sample in sampleNames:
  if 'ggH_hww' not in sample: continue
  if 'FWDH' in sample: continue
  if 'GT200' in sample: continue
...
#Merge all bins
command="text2workspace.py Full2016_ggH_HTXS_Stage2.txt -o Full2016_ggH_HTXS_Stage2_onePOI.root -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel --PO verbose "
poi=''
for sample in sampleNames:
  if 'FWDH' in sample: continue
  else: poi ='r_ggH_hww'
...
'''
#Merge some bins
command="text2workspace.py Full2016_ggH_HTXS_Stage2.txt -o Full2016_ggH_HTXS_Stage2_merged.root -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel --PO verbose "
poi=''
for sample in sampleNames:
  if 'ggH_hww' not in sample: continue
  if 'FWDH' in sample: continue
