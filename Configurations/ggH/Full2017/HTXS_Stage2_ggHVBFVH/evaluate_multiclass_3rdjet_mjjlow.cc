...
#include <iostream>
#include <TMath.h>
#include "generated_code_3rdjet_mjjlow.h"

class evaluate_multiclass_3rdjet_mjjlow : public multidraw::TTreeFunction {
public:
  evaluate_multiclass_3rdjet_mjjlow(int const nclass);

  char const* getName() const override { return "evaluate_multiclass_3rdjet_mjjlow"; }
  TTreeFunction* clone() const override { return new evaluate_multiclass_3rdjet_mjjlow(nclass_); }
...
evaluate_multiclass_3rdjet_mjjlow::evaluate_multiclass_3rdjet_mjjlow(int const nclass) :
  TTreeFunction()
{
  nclass_ = nclass;
}
...
double
evaluate_multiclass_3rdjet_mjjlow::evaluate(unsigned)
{
  ...
  input[28] = qgl->At(jetid0);
  input[29] = qgl->At(jetid1);

  return guess_digit_mjjlow(input, nclass_);;
}

void
evaluate_multiclass_3rdjet_mjjlow::bindTree_(multidraw::FunctionLibrary& _library)
{
