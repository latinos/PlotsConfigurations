...
smp_fake = [skey for skey in samples if 'FAKE' in skey]
data_dr = smp_fake + ['DATA']
mc = [skey for skey in samples if skey not in data_dr]
print('mc samples: '+str(mc))
...
#fr_folder = '/src/PlotsConfigurations/Configurations/monoHWW/SemiLep/Full2017_v7/2HDMa/FR_HT/'
fr_folder = '/src/PlotsConfigurations/Configurations/monoHWW/SemiLep/Full2017_v7/2HDMa/FReleTrig/'
for et in [25, 30, 35]:
    fake_key = 'FAKE_Et'+str(et)
    for lep in ['El', 'Mu']:
        for dEt in [-10, 0, 10]:
            if lep == 'El':
                el_et = et + dEt
            ...
            el_fr_file = os.getenv('CMSSW_BASE') + fr_folder + "plot_ElCh_JetEt"+str(el_et)+"_l1_etaVpt_ptel_fw_ewk_2D.root"
            mu_fr_file = os.getenv('CMSSW_BASE') + fr_folder + "plot_MuCh_JetEt"+str(mu_et)+"_l1_etaVpt_ptmu_fw_ewk_2D.root"
...
for et in [25, 30, 35]:
    ...
    for var in ['Up', 'Down']:
        for lep in ['El', 'Mu']:
            ...
            aliases['FW_mu'+str(et)+'_el'+str(et)+'_'+lep+var] = {
                'expr': '('+is_lep+'*(FW_mu'+str(mu_et)+'_el'+str(el_et)+'[0]/FW_mu'+str(et)+'_el'+str(et)+'[0]) + '+no_lep+')',
                'samples': smp_fake
            }
            aliases['FW_mu'+str(et)+'_el'+str(et)+'_stat'+lep+var] = {
                'expr': '('+is_lep+'*(FW_mu'+str(et)+'_el'+str(et)+'_stat'+var+'[0]/FW_mu'+str(et)+'_el'+str(et)+'[0]) + '+no_lep+')',
                'samples': smp_fake
            }
