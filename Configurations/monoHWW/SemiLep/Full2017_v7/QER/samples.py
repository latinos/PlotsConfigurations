...
#addSampleWeight(samples, 'Wjets', 'WJetsToLNu-1J', '(LHE_Vpt < 50)') 
#addSampleWeight(samples, 'Wjets', 'WJetsToLNu-2J', '(LHE_Vpt < 50)') 
...
for et in [25, 30, 35]:
    ...
    for _, sd in DataRun:
      for pd in DataSets:
        ...
        samples[fake_key]['name'].extend(files)
        samples[fake_key]['weights'].extend([DataTrig[pd]] * len(files))
