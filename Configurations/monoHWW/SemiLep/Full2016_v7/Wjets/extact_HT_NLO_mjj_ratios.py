...
for year in run2:
    ...
    if year == '2016': 
        lo_samp = 'Wjets_HTkf1D'
        nlo_samp = 'Wjets_NLOmerge'
...
# Make pretty
for year in run2:
    n_bins = year_dict[year]['ratio'].GetNbinsX()
    ...
    bin_l_up = year_dict[year]['ratio_s_up'].GetBinContent(n_bins)
    bin_l_do = year_dict[year]['ratio_s_do'].GetBinContent(n_bins)
    year_dict[year]['ratio_s_up'].Smooth()
    year_dict[year]['ratio_s_do'].Smooth()

    ## Reset first and last bin
    #year_dict[year]['ratio_s'].SetBinContent(1, year_dict[year]['ratio'].GetBinContent(1))
    #year_dict[year]['ratio_s'].SetBinContent(n_bins, year_dict[year]['ratio'].GetBinContent(n_bins))
    #year_dict[year]['ratio_s_up'].SetBinContent(1, bin_1_up)
    #year_dict[year]['ratio_s_do'].SetBinContent(1, bin_1_do)
    #year_dict[year]['ratio_s_up'].SetBinContent(n_bins, bin_l_up)
    #year_dict[year]['ratio_s_do'].SetBinContent(n_bins, bin_l_do)
...
for year in run2:
    ...
    if first:
        ...
        year_dict[year]['ratio_s'].GetYaxis().SetRangeUser(0.8, 1.8)
        #year_dict[year]['ratio_s'].GetYaxis().SetRangeUser(0.85, 2.)
...
# Store
file_name = '../../Wjets_kfactors/HT_to_NLO_QCD_k_factors_mjj.root'
