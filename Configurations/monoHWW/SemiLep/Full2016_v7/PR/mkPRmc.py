...
for cut in clean_cuts:
    for var in variables:
        h_name_total     = '_'.join(['h', cut, var])

        # Prepare totals
        for sample in samples:
            if not 'DY' in sample: continue
        ...
        # Calculate fw
        histograms_pr[h_name_total] = copy.deepcopy(histograms_t[h_name_total])
        histograms_pr[h_name_total].Divide(histograms_l[h_name_total])
        ...
        if ':' in variables[var]['name']:
            if len(variables[var]['range']) == 6:
                time.sleep(0.01)
                histograms_pr[h_name_total+'_2D'] = re_roll_2Dh(
...
#            histograms_pr[h_name].SetLineColor(color + idx)
#            histograms_pr[h_name].GetYaxis().SetRangeUser(0., .8)
#            jet_et_str = 'Recoiling jet E_{T} > '+ jet_et.replace('JetEt', '') + ' GeV'
#            legend.AddEntry(histograms_pr[h_name], jet_et_str, 'l')
#            title = ''
#            if 'MuCh' in key: title += 'Muon'
#            else: title += 'Electron'
#            title += ' fake rate 2018' 
#            histograms_pr[h_name].SetTitle(title)
#            if 'l1_pt' in var: histograms_pr[h_name].GetXaxis().SetTitle('p_{T}')
#            elif 'l1_etaVpt' in var: histograms_pr[h_name].GetXaxis().SetTitle('bin')
#            else: histograms_pr[h_name].GetXaxis().SetTitle('#eta')
#            histograms_pr[h_name].GetYaxis().SetTitle('fake rate')
#            if idx == 0: histograms_pr[h_name].Draw()
#            else: histograms_pr[h_name].Draw('same')
#        legend.Draw('same')
#        canvas.Update()
