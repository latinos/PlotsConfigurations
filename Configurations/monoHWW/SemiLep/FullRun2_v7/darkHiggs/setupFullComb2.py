...
pwd = os.getcwd()
semi_lep_dirs = {
    '2016': os.getenv('CMSSW_BASE') + '/src/PlotsConfigurations/Configurations/monoHWW/SemiLep/Full2016_v7/darkHiggs/darkHiggs_lim_datacards_reduced/', 
    '2017': os.getenv('CMSSW_BASE') + '/src/PlotsConfigurations/Configurations/monoHWW/SemiLep/Full2017_v7/darkHiggs/darkHiggs_lim_datacards_reduced/', 
    '2018': os.getenv('CMSSW_BASE') + '/src/PlotsConfigurations/Configurations/monoHWW/SemiLep/Full2018_v7/darkHiggs/darkHiggs_lim_datacards_reduced/', 
}
...
if not args.skip_semi:
    ...
    for year in semi_lep_dirs:
        ...
        if not args.skip_nominal:
            ...
            for region in semi_lep_regions:
                ...
                for mp in eff_mp:
                    ...
                    #print('cp '+in_card+' '+data_card)
                    os.system('cp '+in_card+' '+data_card)
                    rel_path = ''
                    #corr_shapes_path(data_card, pwd+'/'+region_wd)
                    #corr_shapes_path(data_card, rel_path)
        ...
        for mp in eff_mp:
            ...
            for region in semi_lep_regions:
                #region_path = pwd+'/'+year_wd+'/'+region+'/datacard_'+mp+'.txt'
            ...
            #prep_card_thread(out_card, cmd_str, target_dir) 
            #exec_combine_cmd(cmd_str, target_dir)
    ...
    for mp in eff_mp:
        cmd_str = 'combineCards.py '
        for year in semi_lep_dirs:
            #year_path = pwd+'/'+semi_dir+'/'+year+'/datacard_'+year+'_'+mp+'.txt'
            year_path = year+'/datacard_'+year+'_'+mp+'.txt'
            cmd_str+= 'Year'+year+'='+year_path+' '
        target_dir = pwd+'/'+semi_dir
...
if not args.skip_full:
    ...
    for year in semi_lep_dirs:
        ...
        if not args.skip_nominal:
            ...
            for region in full_lep_regions:
                ...
                for mp in eff_mp:
                    ...
                    if in_card is None: 
                        print('      No card found for '+mp+ '  --> Skipping')
                        miss_mp.append(mp)
                        continue
                    #print('cp '+in_card+' '+data_card)
                    os.system('cp '+in_card+' '+data_card)
                    #corr_shapes_path(data_card, region_wd)

                    all_year_cards.append(data_card)
                    #if args.fix_lnN:
                    #    nuisances = ['CMS_PU_'+year, 'CMS_scale_e_'+year, 'CMS_scale_m_'+year]
                    #    #change_shape_2_lnN(data_card, nuisances)        
                    #    th_key = mp
                    #    shape_threads[th_key] = MyThread(target=change_shape_2_lnN, args=(data_card, nuisances,))
        ...
        for mp in eff_mp:
            ...
            for region in full_lep_regions:
                ...
                if not os.path.isfile(region_path_abs): do_mp = False
                cmd_str+= region+'='+region_path+' '
            if not do_mp: continue
            target_dir = pwd+'/'+year_wd
            out_card = 'datacard_'+year+'_'+mp+'.txt'
            cmd_str+= '> '+out_card
    ...
    for mp in eff_mp:
        ...
        for year in full_lep_dirs:
            year_path = year+'/datacard_'+year+'_'+mp+'.txt'
            year_path_abs = pwd+'/'+full_dir+'/'+year_path
            if not os.path.isfile(year_path_abs): do_mp = False
            cmd_str+= 'Year'+year+'='+year_path+' '
        if not do_mp: continue
        target_dir = pwd+'/'+full_dir
        out_card = 'datacard_run2_'+mp+'.txt'
        cmd_str+= '> '+out_card
        #exec_combine_cmd(cmd_str, target_dir)
        #add_ranges_bkgnorm(target_dir+'/'+out_card)
...
for mp in eff_mp:
    ...
    if not os.path.isfile(semi_card): continue
    full_card = pwd+'/'+full_dir+'/'+in_card
    if not os.path.isfile(full_card): continue
