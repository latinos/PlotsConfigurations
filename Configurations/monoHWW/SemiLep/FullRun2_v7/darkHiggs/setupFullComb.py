...
def change_shape_2_lnN(data_card, nuisances):
...
if not args.skip_semi:
    ...
    for th_key in threads:
        ...
        if not threads[th_key].err is None:
            raise RuntimeError('Thread "'+th_key+'" failed with: '+threads[th_key].err)
...
if not args.skip_full:
    ...
    for mp in eff_mp:
        ...
        for year in full_lep_dirs:
            #year_path = pwd+'/'+full_dir+'/'+year+'/datacard_'+year+'_'+mp+'.txt'
            year_path = year+'/datacard_'+year+'_'+mp+'.txt'
            cmd_str+= 'Year'+year+'='+year_path+' '
...
for mp in eff_mp:
    ...
    cmd_str = 'combineCards.py Semi='+pwd+'/'+semi_dir+'/'+in_card+' Full='+pwd+'/'+full_dir+'/'+in_card
