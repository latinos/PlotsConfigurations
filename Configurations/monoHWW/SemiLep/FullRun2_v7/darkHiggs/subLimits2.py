...
pwd = os.getcwd()
year_dict = {
    '2016': os.getenv('CMSSW_BASE') + '/src/PlotsConfigurations/Configurations/monoHWW/SemiLep/Full2016_v7/darkHiggs/darkHiggs_lim_datacards'+args.tag+'/', 
    '2017': os.getenv('CMSSW_BASE') + '/src/PlotsConfigurations/Configurations/monoHWW/SemiLep/Full2017_v7/darkHiggs/darkHiggs_lim_datacards'+args.tag+'/', 
    '2018': os.getenv('CMSSW_BASE') + '/src/PlotsConfigurations/Configurations/monoHWW/SemiLep/Full2018_v7/darkHiggs/darkHiggs_lim_datacards'+args.tag+'/', 
}
...
work_dir = work_dir_base + '/' + args.var.replace('YEAR', 'Run2')
...
job_dir = job_dir_base + '/' + args.var.replace('YEAR', 'Run2')
...
if not args.resub: 
    if not args.no_cp:
        ...
        for year in run2:
            ...
            for idx, ch in enumerate(channels):
                ...
                yr_channel_path = base_dir + '/' +yr_channel + '/' + channel_vars[idx].replace('YEAR', year)
                if not os.path.isdir(yr_channel_path): os.system('mkdir '+yr_channel_path)
                channel_path = year_dict[year] + ch + '/' + channel_vars[idx].replace('YEAR', year)
    ...
    all_files = []
    for year in run2:
        for fil in os.listdir(base_dir+'/'+year+'_InCh_Combi/'+args.var.replace('YEAR', year)):
            if not fil.startswith('datacard') : continue
            if not fil.endswith('.txt'): continue
    ...
    for fil in all_files:
        ...
        f2016_path = base_dir + '/2016_InCh_Combi/' + args.var.replace('YEAR', '2016') + '/' + fil    
        f2017_path = base_dir + '/2017_InCh_Combi/' + args.var.replace('YEAR', '2017') + '/' + fil    
        f2018_path = base_dir + '/2018_InCh_Combi/' + args.var.replace('YEAR', '2018') + '/' + fil    
    
        f2016_path_rel = '../../2016_InCh_Combi/' + args.var.replace('YEAR', '2016') + '/' + fil    
        f2017_path_rel = '../../2017_InCh_Combi/' + args.var.replace('YEAR', '2017') + '/' + fil    
        f2018_path_rel = '../../2018_InCh_Combi/' + args.var.replace('YEAR', '2018') + '/' + fil    
