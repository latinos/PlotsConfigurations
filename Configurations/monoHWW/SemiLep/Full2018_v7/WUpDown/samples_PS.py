...
mcCommonWeightNoMatch = 'PUJetIdSF[0]'
#mcCommonWeightNoXS    =          'METFilter_MC*PUJetIdSF[0]*PromptGenLepMatch1l'
mcCommonWeight        = 'PUJetIdSF[0]*PromptGenLepMatch1l'
...
#ISR up='PSWeight[2]', do='PSWeight[0]'
#FSR up='PSWeight[3]', do='PSWeight[1]' 

curr_samps = samples.keys()
for samp in curr_samps:
    if not 'weight' in samples[samp]: continue
    samples[samp+'_ISR_Up'] = copy.deepcopy(samples[samp])
    samples[samp+'_ISR_Up']['weight'] += ' * (PSWeight[2])'
    samples[samp+'_ISR_Down'] = copy.deepcopy(samples[samp])
    samples[samp+'_ISR_Down']['weight'] += ' * (PSWeight[0])'
    samples[samp+'_FSR_Up'] = copy.deepcopy(samples[samp])
    samples[samp+'_FSR_Up']['weight'] += ' * (PSWeight[3])'
    samples[samp+'_FSR_Down'] = copy.deepcopy(samples[samp])
    samples[samp+'_FSR_Down']['weight'] += ' * (PSWeight[1])'
