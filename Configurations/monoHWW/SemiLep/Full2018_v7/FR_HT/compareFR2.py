...
comp_dict = {
    'prev': {
        'file': '../2HDMa/FR_root/plots_FR_2018v7.root',
        },
    'curr': {
        'file': 'FR_root/plots_FR_HT_2018v7.root',
        },
}
#
...
for it in comp_dict:
    r_file_n = comp_dict[it]['file']
    r_file[it] = ROOT.TFile(r_file_n)
    ...
    for cut in cuts:
        ...
        for var in variables:
            for sample in samples:
                ...
                if is_t: histograms_t[it][h_name] = r_file[it].Get(cut+'/'+var+'/'+get_name)
                if is_l: histograms_l[it][h_name] = r_file[it].Get(cut+'/'+var+'/'+get_name)
    ...
    for cut in clean_cuts:
        for var in variables:
            ...
            for sample in samples:
                ...
                histograms_t[it][h_name_total]     = copy.deepcopy(histograms_t[it][h_name]) 
                histograms_t[it][h_name_total_ewk] = copy.deepcopy(histograms_t[it][h_name]) 
                histograms_l[it][h_name_total]     = copy.deepcopy(histograms_l[it][h_name]) 
                histograms_l[it][h_name_total_ewk] = copy.deepcopy(histograms_l[it][h_name]) 
    
            # Substract ewk
            for sample in samples:
                if 'DATA' in sample: continue
                h_name = '_'.join(['h', cut, var, sample])
                histograms_t[it][h_name_total_ewk].Add(histograms_t[it][h_name], -1.)
                histograms_l[it][h_name_total_ewk].Add(histograms_l[it][h_name], -1.)
     
            # Calculate fw
            histograms_fr[it][h_name_total] = copy.deepcopy(histograms_t[it][h_name_total])
            histograms_fr[it][h_name_total].Divide(histograms_l[it][h_name_total])
    
            if 'MuCh_JetEt10_l1_pt_ptmu' in h_name_total_ewk: 
                bin_l = histograms_l[it][h_name_total_ewk].GetNbinsX()
                bin_t = histograms_t[it][h_name_total_ewk].GetNbinsX()
                h_name = '_'.join(['h', cut, var, 'Wjets'])
                print(h_name_total_ewk, 
                    histograms_t[it][h_name_total_ewk].GetBinContent(bin_t), 
                    histograms_l[it][h_name_total_ewk].GetBinContent(bin_l),
                    histograms_t[it][h_name_total].GetBinContent(bin_t), 
                    histograms_l[it][h_name_total].GetBinContent(bin_l),
                    histograms_t[it][h_name].GetBinContent(bin_l)/histograms_t[it][h_name_total].GetBinContent(bin_t),
                    histograms_l[it][h_name].GetBinContent(bin_l)/histograms_l[it][h_name_total].GetBinContent(bin_t),
                    )
            histograms_fr[it][h_name_total_ewk] = copy.deepcopy(histograms_t[it][h_name_total_ewk])
            histograms_fr[it][h_name_total_ewk].Divide(histograms_l[it][h_name_total_ewk])
    
            #if ':' in variables[var]['name']:
            #    if len(variables[var]['range']) == 6:
            #        time.sleep(0.01)
            #        histograms_fr[it][h_name_total_ewk+'_2D'] = re_roll_2Dh(
            #            histograms_fr[it][h_name_total_ewk], 
            ...
            #            )
            #        time.sleep(0.01)
            #        histograms_fr[it][h_name_total+'_2D'] = re_roll_2Dh(
            #            histograms_fr[it][h_name_total], 
            #            variables[var]['range'][0], variables[var]['range'][1], variables[var]['range'][2], 
            #            variables[var]['range'][3], variables[var]['range'][4], variables[var]['range'][5], 
            ...
            #        time.sleep(0.01)
            #        histograms_fr[it][h_name_total_ewk+'_2D'] = re_roll_2Dh_array(
            #            histograms_fr[it][h_name_total_ewk], 
            ...
            #            )
            #        time.sleep(0.01)
            #        histograms_fr[it][h_name_total+'_2D'] = re_roll_2Dh_array(
            #            histograms_fr[it][h_name_total], 
            #            variables[var]['range'][0], 
            #            variables[var]['range'][1], 
            #            name = 'FW_no_ewk_'+cut+'_'+var,
            ...
            #            )
            #        time.sleep(0.01)
            #        histograms_fr[it][h_name_total_ewk+'_2D_invert'] = re_roll_2Dh_array(
            #            histograms_fr[it][h_name_total_ewk], 
            #            variables[var]['range'][0], 
            #            variables[var]['range'][1], 
            ...
            #            )
            #        time.sleep(0.01)
            #        histograms_fr[it][h_name_total+'_2D_invert'] = re_roll_2Dh_array(
            #            histograms_fr[it][h_name_total], 
            #            variables[var]['range'][0], 
            #            variables[var]['range'][1], 
...
for fw in histograms_fr['curr']:
    ...
    if fw.endswith('_2D'):
        ...
        histograms_fr['curr'][fw].SetTitle(title)
        histograms_fr['curr'][fw].GetYaxis().SetTitle('Fake Rate')
        #histograms_fr['curr'][fw].GetXaxis().SetTitle('')
        #histograms_fr['curr'][fw].GetZaxis().SetRangeUser(.3, .85)
        #canvas.SetLogy()
        histograms_fr['curr'][fw].SetLineColor(4)
        histograms_fr['prev'][fw].SetLineColor(1)
        histograms_fr['prev'][fw].Draw()
        histograms_fr['curr'][fw].Draw('same')
