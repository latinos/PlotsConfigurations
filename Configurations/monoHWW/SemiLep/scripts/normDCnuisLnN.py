...
for nuis in nuis_list:
    ...
    for idx in range(-1, (n_nuis-1)*-1, -1):
        ...
        if args.sample in samp:
            if '-' == val: raise ValueError('Trying to normalize "-"')
            elif '/' in val:
    ...
    for idx,bit in enumerate(nuis_line_s):
        if bit == nuis_line_f[nuis_idx]:
            ...
            new_nuis_line_s.append(new_nuis_line_f[nuis_idx])
            nuis_idx += 1
