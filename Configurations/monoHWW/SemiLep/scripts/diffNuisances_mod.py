...
parser.add_option("-g", "--histogram", dest="plotfile", default=None, type="string", help="If true, plot the pulls of the nuisances to the given file (no ext).")
...
parser.add_option("", "--long", dest="long", default=False, action='store_true', help="Increase canvas size")
...
if options.plotfile:
    import ROOT
    fout = ROOT.TFile(options.plotfile+'.root',"RECREATE")
    ROOT.gROOT.SetStyle("Plain")
    ROOT.gStyle.SetOptFit(1)
    ROOT.gStyle.SetOptStat(0)
    ROOT.gStyle.SetPadBottomMargin(0.20)#0.13)
    ...
    if options.long: canvas_nuis = ROOT.TCanvas("nuisances", "nuisances", 1800, 600)
    else: canvas_nuis = ROOT.TCanvas("nuisances", "nuisances", 900, 600)
    ...
    canvas_nuis.SaveAs(options.plotfile+'.png')
