...
void macroPulls(TString folder = "pulls_em_MVA",
  ...
  TString title = "Pulls for mass point " + massPoint + ": Signal Region (35.9 fb^{-1}) and Control Regions for variable " + variable ;
  if (region == "CR")
    title = "Pulls for mass point " + massPoint + ": Control Regions only for variable " + variable ;
  if (region == "B")
    title = "Pulls for mass point " + massPoint + ": Signal and Control Region Asimov (signal strength = 0) for variable " + variable ;
  if (region == "SB")
    title = "Pulls for mass point " + massPoint + ": Signal and Control Region Asimov (signal strength = 1) for variable " + variable ;
  if (region == "BSR")
    title = "Pulls for mass point " + massPoint + ": Blind Signal Region (2.4 fb^{-1}) and Control Regions for variable " + variable ;
  gr3->SetTitle(title);
  ...
  c2->Print(folder + "/pulls_" + model + massPoint + "_" + variable + "_" + region + ".pdf","pdf");
}
