...
for categories in newfile.GetListOfKeys(): # Cut directory
  ...
  for variables in category.GetListOfKeys(): # Variable directory
    ...
    nominal = variable.Get("histo_Dyemb")
    if not variables.GetName()=="events": continue
    if nominal.Integral()==0 or DYuncDn.Integral()==0: continue
    print "CATEGORY: ",categories.GetName()," VARIABLE: ", variables.GetName(), "Up/Down variation = ", DYuncUp.Integral()/nominal.Integral(),"/",nominal.Integral()/DYuncDn.Integral()
