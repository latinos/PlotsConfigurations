...
#ifndef MVAReader_lowZ_H
#define MVAReader_lowZ_H

class MVAReader_lowZ : public multidraw::TTreeFunction {
public:
  
  MVAReader_lowZ(const char* model_path, bool verbose, int cut);

  char const* getName() const override { return "MVAReader_lowZ"; }
  TTreeFunction* clone() const override { return new MVAReader_lowZ(model_path_.c_str(), verbose, cut_); }
...
MVAReader_lowZ::MVAReader_lowZ(const char* model_path, bool verbose, int cut):
...
double 
MVAReader_lowZ::evaluate(unsigned)
{
  // Run only if 
...
void
MVAReader_lowZ::bindTree_(multidraw::FunctionLibrary& _library)
{  
