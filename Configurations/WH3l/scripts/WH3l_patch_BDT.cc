...
class WH3l_patch_BDT : public multidraw::TTreeFunction 
{
    public:

        WH3l_patch_BDT(const char* variable_);
        static constexpr int BIGNUMBER{9999};
        static constexpr double Z_MASS{91.1876};

        char const* getName() const override { return "WH3l_patch_BDT"; }
        TTreeFunction* clone() const override { return new WH3l_patch_BDT(variable.c_str()); }
...
WH3l_patch_BDT::WH3l_patch_BDT(const char* variable_) :
    TTreeFunction(),
    variable{variable_}
{
...
void
WH3l_patch_BDT::bindTree_(multidraw::FunctionLibrary& _library)
{
...
double
WH3l_patch_BDT::evaluate(unsigned)
{
...
inline float 
WH3l_patch_BDT::bVeto(FloatArrayReader* Jet_btagDeepB, int CleanJet_jetIdx)
{
    //WH3l_patch_BDT::bVeto(float* Jet_btagDeepB, int CleanJet_jetIdx){
...
double WH3l_patch_BDT::get_mOSll(
         ...
         PtEtaPhiMVector const& jLep, int const& jLepPdgId)
{
    if (iLepPdgId * jLepPdgId < 0){
        ...
        return -WH3l_patch_BDT::BIGNUMBER;
    }
}

inline double WH3l_patch_BDT::get_ptOSll(
        PtEtaPhiMVector const& iLep, int const& iLepPdgId,
        PtEtaPhiMVector const& jLep, int const& jLepPdgId)
{
    if (iLepPdgId * jLepPdgId < 0){
...
inline double WH3l_patch_BDT::get_drOSll(
         PtEtaPhiMVector const& iLep, int const& iLepPdgId,
         PtEtaPhiMVector const& jLep, int const& jLepPdgId)
{
    if (iLepPdgId * jLepPdgId < 0){
         ...
         std::vector<PtEtaPhiMVector> lep, std::vector<int > lepPdgId)
{
    ...
    return minmllDiffToZ != WH3l_patch_BDT::BIGNUMBER ? minmllDiffToZ : -WH3l_patch_BDT::BIGNUMBER;
}

inline double WH3l_patch_BDT::get_ptlll(
...
inline double WH3l_patch_BDT::get_mlll(
         PtEtaPhiMVector const& iLep,
         PtEtaPhiMVector const& jLep,
         PtEtaPhiMVector const& kLep)
{
...
inline double WH3l_patch_BDT::get_mtlmet(
...
inline double WH3l_patch_BDT::get_dphilmet(
         PtEtaPhiMVector const& lep,
         PtEtaPhiMVector const& met)
{
...
inline double WH3l_patch_BDT::get_ptWWW(
         PtEtaPhiMVector const& iLep,
         PtEtaPhiMVector const& jLep,
         PtEtaPhiMVector const& kLep,
         PtEtaPhiMVector const& met)
...
inline double WH3l_patch_BDT::get_mtWWW(
         PtEtaPhiMVector const& iLep,
         PtEtaPhiMVector const& jLep,
         PtEtaPhiMVector const& kLep,
         PtEtaPhiMVector const& met)
...
inline double WH3l_patch_BDT::get_dphilllmet(
         PtEtaPhiMVector const& iLep,
         PtEtaPhiMVector const& jLep,
         PtEtaPhiMVector const& kLep,
         PtEtaPhiMVector const& met)
...
double
WH3l_patch_BDT::getValue(std::string variableName)
{
...
void
WH3l_patch_BDT::initMvaReader_BDTGSSSF()
{
    ...
    mvaReader_BDTGSSSF->BookMVA("BDTGC10D4C10S1","/afs/cern.ch/user/p/pyu/public/HWWAnalysis/FullRunII06Apr_v6/TMVAClassification_2016SSSF.weights.xml");

}

double
WH3l_patch_BDT::get_BDTGSSSF()
{
...
void
WH3l_patch_BDT::initMvaReader_BDTGOSSF()
{
    ...
    mvaReader_BDTGOSSF->BookMVA("BDTG4F07","/afs/cern.ch/user/p/pyu/public/HWWAnalysis/FullRunII06Apr_v6/TMVAClassification_2016OSSF.weights.xml");
}

double
WH3l_patch_BDT::get_BDTGOSSF()
{
    loc_WH3l_dphilllmet = getValue("dphilllmet") ;
    loc_WH3l_mOSll_min  = getValue("mOSllmin")   ;
    loc_WH3l_ptOSll_min = getValue("ptOSllmin")  ;
    ...
    float classifier = mvaReader_BDTGOSSF->EvaluateMVA("BDTG4F07");
    return classifier;
}
